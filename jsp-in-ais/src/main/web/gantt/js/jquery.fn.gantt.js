...
(function ($, undefined) {
    ...
    function dataScroll() {
        var a = document.getElementById("dataPanelDataContainer").scrollTop;
        var b = document.getElementById("dataPanelDataContainer").scrollLeft;
        document.getElementById("leftPanelDataContainer").scrollTop = a;
        document.getElementById("dataPanelCaptionContainer").scrollLeft = b;
    }
    ...
    $.fn.gantt = function (options) {
        ...
        var core = {
            ...
            init: function (element) {
                ...
                var diffDays = (element.dateEnd-element.dateStart) / (1000 * 60 * 60 * 24);
                if (settings.scale == 'weeks'){
                    if (diffDays<366){
                        element.dateEnd = new Date(element.dateEnd.getFullYear(), 12, 31);
                    }
                }else if (settings.scale == 'days'){
                    if (diffDays<70){
                        element.dateEnd = new Date(element.dateEnd.setDate(element.dateEnd.getDate()+(70-diffDays)));
                    }
                }
            ...
            render: function (element) {
                ...
                var $dataPanelData = $('<div class="dataPanelData"/>').css("height", tools.getCellSize() * element.data.length).css("width", $(".dataPanelCaption").width());

                var containerWidth = $("body").width() - LEFT_PANEL_WIDTH;//$(".fn-gantt .leftPanel").width();
                if ($(".dataPanelCaption").width() < containerWidth){
                    containerWidth = $(".dataPanelCaption").width();
                }
                var containerHeight = $(".gantt").height() - tools.getCellSize() * element.headerRows;
                //高度填充满
                /*if (tools.getCellSize() * element.data.length < containerHeight){
                    containerHeight = tools.getCellSize() * element.data.length;
                }*/
                var dataPanelDataContainer = $('<div id="dataPanelDataContainer" style="overflow: auto;"/>').css("width", containerWidth).css("height", containerHeight);
                dataPanelDataContainer.append($dataPanelData);

                $dataPanel.append(dataPanelDataContainer);

                //core.fillData(element, $dataPanel, $leftPanel);
                core.fillData(element, $dataPanelData, $leftPanel);
                ...
                $leftPanel.css({ height: $dataPanel.height() });
                core.waitToggle(element);
                settings.onRender();

                if (dataPanelDataContainer && dataPanelDataContainer[0]) {
                    if (hasScrolled(dataPanelDataContainer[0], VERTICAL)) {
                        var scrollBarWidth = dataPanelDataContainer[0].offsetWidth - dataPanelDataContainer[0].clientWidth;
                        $("#dataPanelCaptionContainer").css("width", $("#dataPanelCaptionContainer").width() - scrollBarWidth);
                    }
                    if (hasScrolled(dataPanelDataContainer[0], HORIZONTAL)) {
                        var scrollBarHeight = dataPanelDataContainer[0].offsetHeight - dataPanelDataContainer[0].clientHeight;
                        //如果需要才减去滚动条的高度
                        if ($leftPanel.height()-$(".leftPanelCaption").height()-$("#leftPanelDataContainer").height()<scrollBarHeight){
                            $("#leftPanelDataContainer").css("height", $("#leftPanelDataContainer").height() - scrollBarHeight);
                        }
                    }
                }

                $("#dataPanelDataContainer").scroll(function () {
                    dataScroll();
                })
            },
            ...
            leftPanel: function (element) {
				...
				*/
				var captionRowCount = element.headerRows/2;
/*                var ganttLeftPanel = $('<div class="leftPanel"/>')
                    .append(
						$('<div class="row ganttCaption"/>').css("height", tools.getCellSize() * captionRowCount)
						.append(
							$('<span class="fn-label">' + settings.caption + '</span>')
						)
					);	*/

                var ganttLeftPanel = $('<div class="leftPanel"/>');
                var ganttLeftPanelCaption = $('<div class="leftPanelCaption"/>').css("height", tools.getCellSize() * element.headerRows);
                var ganttLeftPanelData = $('<div class="leftPanelData"/>').css("height", tools.getCellSize() * element.rowsNum);

                ganttLeftPanelCaption.append(
                    $('<div class="row ganttCaption"/>').css("height", tools.getCellSize() * captionRowCount)
                        .append(
                            $('<span class="fn-label">' + settings.caption + '</span>')
                        )
				...
				var columnCaptionHeight = tools.getCellSize() * (element.headerRows-captionRowCount);
				/*
				ganttLeftPanel.append(
					$('<div class="row spacer"/>').css("height", tools.getCellSize() * (element.headerRows-1))
					.append($('<div class="fn-label">' + settings.nameCaption + '</div>'))
					.append($('<div class="fn-label">' + settings.statusCaption + '</div>'))
					.append($('<div class="fn-label">' + settings.descCaption + '</div>'))
				);
				*/

                ganttLeftPanelCaption.append($('<div class="row nameCaption"><span class="fn-label">' + settings.nameCaption + '</span></div>').css("height", columnCaptionHeight))
				.append($('<div class="row statusCaption"><span class="fn-label">' + settings.statusCaption + '</span></div>').css("height", columnCaptionHeight))
				.append($('<div class="row descCaption"><span class="fn-label">' + settings.descCaption + '</span></div>').css("height", columnCaptionHeight));
				...
				*/
                ganttLeftPanelData.append(entries.join(""));

                // debugger;
                var containerHeight = $(".gantt").height() - tools.getCellSize() * element.headerRows;
                //如果内容的小就取内容的高度
                if (tools.getCellSize() * element.rowsNum < containerHeight){
                    containerHeight = tools.getCellSize() * element.rowsNum;
                }
                var leftPanelDataContainer = $('<div id="leftPanelDataContainer" style="overflow: hidden;"/>').css("height", containerHeight);
                leftPanelDataContainer.append(ganttLeftPanelData);

                //return ganttLeftPanel.append(entries.join(""));
                return ganttLeftPanel.append(ganttLeftPanelCaption).append(leftPanelDataContainer);
            },
            ...
            dataPanel: function (element, width) {
                ...
                var dataPanel = $('<div class="dataPanel"/>').css("height", $(".gantt").height());

                var containerWidth = $("body").width() - LEFT_PANEL_WIDTH;
                //如果实际的宽度小就取实际的
                if (width < containerWidth){
                    dataPanel.css("width", width);
                }
            ...
            rightPanel: function (element, leftPanel /* <- never used? */) {
                ...
                var dataPanelCaption;
                dataPanelCaption = $('<div class="dataPanelCaption"></div>').css("height", tools.getCellSize() * element.headerRows);
                ...
                case "weeks":
                    ...
                    for (i = 0, len = range.length; i < len; i++) {
                        ...
                        // Fill years
                        curWeek = rday.getWeekOfYear();

                        //12月的最后一周可能获取的week=1
                        var bLastWeekOfYear = ((i == 0) && (week == 1) && (month == 11));

                        //if (week > (week = rday.getWeekOfYear())) {
                        if (bLastWeekOfYear || week > curWeek) {
                ...
                default:
                    ...
                    for (i = 0, len = range.length; i < len; i++) {
                        ...
                        // Fill months
                        if (rday.getMonth() !== month) {
                            var style = scaleUnitsThisMonth == 0 ? 'display:none;' : 'width:' + tools.getCellSize() * scaleUnitsThisMonth + 'px;';
                            monthArr.push(
                                '<div class="row month" style="' +
                                style +
                                '"><div class="fn-label">' +
                                settings.months[month] +
                                '</div></div>');
                ...
                dataPanelCaption.css("width", dataPanelWidth);

                var containerWidth = $("body").width() - LEFT_PANEL_WIDTH;//$(".fn-gantt .leftPanel").width();
                //如果实际的宽度小就取实际的
                if (dataPanelWidth < containerWidth){
                    containerWidth = dataPanelWidth;
                }
                var dataPanelCaptionContainer = $('<div id="dataPanelCaptionContainer" style="overflow: hidden;"/>').css("width", containerWidth);
                dataPanelCaptionContainer.append(dataPanelCaption);

                dataPanel.append(dataPanelCaptionContainer);
                ...
                if (desc) {
                    bar
                      ...
                      .mousemove(function (e) {
                          ...
                          $(".fn-gantt-hint").css("top", e.pageY + 10);
                      });
                }
                if (classNames) {
                    ...
                    if (i >= element.pageNum * settings.itemsPerPage &&
                        ...
                        $.each(entry.values, function (j, day) {
                            ...
                            case "weeks":
                                ...
                                // find row
                                //计划行（偶数行）和实际行（奇数行）caption合并了，实际的找不到对应的rowheader了，此处减一找对应的计划，然后再加上cellWidth
                                var topOffset;
                                if (i%2 == 1){
                                    var oddRow = i-1;
                                    topEl = $(element).find("#rowheader" + oddRow);
                                    topOffset = topEl.data("offset") + cellWidth;
                                }else{
                                    var evenRow = i;
                                    topEl = $(element).find("#rowheader" + evenRow);
                                    topOffset = topEl.data("offset");
                                }
                                //topEl = $(element).find("#rowheader" + i);
                                //top = cellWidth * 3 + barOffset + topOffset;
                                top = barOffset + topOffset;
                                _bar.css({
                                  top: top,
                                  left: Math.floor(cFrom),
                                  width: dp + '%'
                            ...
                            case "months":
                                ...
                                //top = cellWidth * 2 + barOffset + topOffset;
                                top = barOffset + topOffset;
                                _bar.css({
                                  top: top,
                                  left: Math.floor(cFrom),
                            ...
                            default:
                                ...
                                //top = cellWidth * 4 + barOffset + topOffset;
                                top = barOffset + topOffset;
                                _bar.css({
                                  top: top,
                                  left: Math.floor(cFrom),
        ...
        var tools = {
            ...
            getMaxDate: function (element) {
                ...
                case "weeks":
                    ...
                    //今年如果还剩余小于3周则往后取3周，否则修改为默认展示全年的 by zxl
                    if (bd.getFullYear()!=settings.year || (bd.getMonth()==12 && bd.getDate()>10)){
                        bd = new Date(bd.setDate(bd.getDate() + 3 * 7));
                        var md = Math.floor(bd.getDate() / 7) * 7;
                        maxDate = new Date(bd.getFullYear(), bd.getMonth(), md === 0 ? 4 : md - 3);
                    }else{
                        maxDate = new Date(bd.getFullYear(), 12, 31);
                    }
                    break;
                case "months":
                    ...
                    //修改为默认展示全年的 by zxl   月视图再加2年，为了显示好看
                    if (bd.getMonth()>10){
                        bd.setMonth(bd.getMonth() + 2);
                        maxDate = new Date(bd.getFullYear()+2, bd.getMonth(), 1);
                    }else{
                        maxDate = new Date(bd.getFullYear()+2, 12, 1);
                    }
                    break;
                case "days":
                    /* falls through */
                default:
                    maxDate.setHours(0);
                    if (maxDate.getFullYear()==settings.year && maxDate.getMonth()==0) {
            ...
            getMinDate: function (element) {
                ...
                case "weeks":
                    ...
                    //今年如果小于3周则往前取3周，否则修改为默认展示全年的 by zxl
                    if (bd.getFullYear()!=settings.year || (bd.getMonth()==1 && bd.getDate()< 3*7)){
                        bd = new Date(bd.setDate(bd.getDate() - 3 * 7));
                        var md = Math.floor(bd.getDate() / 7) * 7;
                        minDate = new Date(bd.getFullYear(), bd.getMonth(), md === 0 ? 4 : md - 3);
                    }else{
                ...
                default:
                    minDate.setHours(0, 0, 0, 0);
                    if (minDate.getFullYear()==settings.year && minDate.getMonth()==11){
