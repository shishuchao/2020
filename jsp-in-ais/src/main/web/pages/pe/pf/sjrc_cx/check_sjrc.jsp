			...
			<tr class="listtablehead">
				<td colspan="4" align="left" class="edithead">
					<span style="display: inline; width: 80%;float: left;">
						<s:property escape="false" value="@ais.framework.util.NavigationUtil@getNavigation('/ais/pe/pf/sjrc_cx/check.action?peObjectCode=1000203')"/>
					</span>
	 ...
	 <s:doubleselect 
      ...
      theme="ufaud_2" templateDir="/strutsTemplate">
				...
				<tr class="listtablehead">
					...
					<TD class=listtabletr1>
					<s:buttonText name="sco.department" 
							hiddenName="sco.departmentCode" 
							cssStyle="width:60%" 
							doubleOnclick="showPopWin('/pages/system/search/searchdata.jsp?url=/system/uSystemAction!morgList.action&paraname=sco.department&paraid=sco.departmentCode',600,450,'组织机构选择')" 
					...
					<td colspan="6" align="right" class="listtabletr1">
						<div align="right">
							...
							<input type="button" value="重置"
								onclick="window.location.href='${contextPath}/pe/pf/sjrc_cx/check.action?peObjectCode=1000203'" />
						</div>
					</td>
				</tr>
			</TABLE>
		...
		<s:form namespace="/pe/pf/sjrc_cx" name="myforml" action="check" method="post">
			...
			<center>
				<display:table requestURI="${contextPath}/pe/pf/sjrc_cx/check.action" name="scoList" id="row" class="its" pagesize="10" >
					<display:column property="object_name" title="姓名" headerClass="center" sortable="true"/>								
					...
					<s:a cssStyle="cursor:hand"  onclick="openWindowByUrl_common('${contextPath}/pe/pf/sjrc_cx/getMore.action?id=${row.id}&tag=sjrc')"><s:div cssStyle="text-align:center; color:gray">详细信息</s:div></s:a>			
					</display:column>                                                            
				</display:table>
				<br>
...
function doSome(bool){
    ...
    if(!numberValidate_out('sco.object_name',50,'姓名')){      
      return false;
    }else{
	 document.getElementById('export').value=bool;	
	 return true;
