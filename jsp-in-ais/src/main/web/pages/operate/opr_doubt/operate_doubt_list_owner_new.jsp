	...
	<head>
		...
		Ext.onReady(function(){
        	...
        	<%}%>
			 ...
			 //疑点grid
	         var dataModelDoubt={url:'${pageContext.request.contextPath}/operate/doubtExt/doubtListJsonNew.action?taskId=<%=request.getParameter("taskId")%>&permission=<%=request.getParameter("permission")%>&isView=<%=request.getParameter("isView")%>&project_id=<%=request.getParameter("project_id")%>',
	        ...
	        var gridDoubt =new Usp.Grid({
	            gridConfig:{title:'审计疑点',id:'common-data-datadetaillist-g2<%=owner%>',collapsible:true,autoHeight:false,height:window.screen.availHeight-260,collapsed:
	            false,
