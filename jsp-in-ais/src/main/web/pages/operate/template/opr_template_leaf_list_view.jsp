	...
	<head>
		...
		<script type="text/javascript">
           	...
           	function add()
		  {
		     ...
		     showPopWin('${contextPath}/operate/template/addLeaf.action?tab=proc&type='+type+'&taskTemplateId=<%=request.getParameter("taskTemplateId")%>&audTemplateId=<%=request.getParameter("audTemplateId")%>',720,600,title);
		     var num=Math.random();
		  ...
		  { 
			 window.open("${contextPath}/operate/template/showContentLeaf.action?taskTemplateId="+s,"","height=700px, width=800px, toolbar=no, menubar=no, scrollbars=no,resizable=yes,location=no, status=no");
		   } 
		     //删除
          function deleteLeafList(s,q)
		  { 
		  if(confirm("确认删除吗？")){
			 window.location.href="${contextPath}/operate/template/deleteLeafList.action?pid="+q+"&taskTemplateId="+s;
			} 
		   } 
     </script>
		<link href="${contextPath}/styles/main/ais.css" rel="stylesheet"
	...
	<body>
		<center>
			<form id="myform" name="my_form" onsubmit="return true;"
				action="/ais/operate/template/showContentLeafListView.action" method="post" style="">
				<s:hidden name="pro_id" />
				<s:hidden name="taskPid" />
				<s:hidden name="taskId" />
				<display:table name="list" id="row"
					requestURI="${contextPath}/operate/template/showContentLeafListView.action" class="its"
