...
<html>
	...
	<body class="easyui-layout" border="0">
		<div region="center" border="0">
			...
			<div id='main' title='总体方案' border="0" <% if(StringUtils.isNotBlank(request.getParameter("selectedTab")) && request.getParameter("selectedTab").equals("main")){%> selected="true"<%} %> style="overflow:hidden;">
				<iframe id="basefrm1"
					src="${contextPath}/operate/template/showContentRootView.action?interCtrl=${interCtrl}&node=${node}&path=${path}&selectedTab=main&taskTemplateId=<%=request.getParameter("taskTemplateId")%>&audTemplateId=<%=request.getParameter("audTemplateId")%>"
            ...
            <s:if test="'1' == '${type}'">
			   <div id='item' title='审计事项' <% if(StringUtils.isNotBlank(request.getParameter("selectedTab")) && request.getParameter("selectedTab").equals("item")){%> selected="true"<%} %> style="overflow:hidden;">
					<iframe id="basefrm2"
					src="${contextPath}/operate/template/showContentTypeView.action?interCtrl=${interCtrl}&node=<%=request.getParameter("node")%>&path=<%=request.getParameter("path")%>&selectedTab=item&type=<%=request.getParameter("type")%>&taskPid=<%=request.getParameter("taskPid")%>&taskTemplateId=<%=request.getParameter("taskTemplateId")%>&audTemplateId=<%=request.getParameter("audTemplateId")%>"
					frameborder="0" width="100%" height="100%" onload="doAutoHeight2()"></iframe>
			    </div>
			</s:if>
		     <s:if test="'2' == '${type}'">
				<div  id='item' title='审计事项' <% if(StringUtils.isNotBlank(request.getParameter("selectedTab")) && request.getParameter("selectedTab").equals("item")){%> selected="true"<%} %> style="overflow:hidden;">
					<iframe   id="basefrm2"
					src="${contextPath}/operate/template/showContentLeafView.action?interCtrl=${interCtrl}&node=<%=request.getParameter("node")%>&path=<%=request.getParameter("path")%>&selectedTab=item&type=<%=request.getParameter("type")%>&taskPid=<%=request.getParameter("taskPid")%>&taskTemplateId=<%=request.getParameter("taskTemplateId")%>&audTemplateId=<%=request.getParameter("audTemplateId")%>"
					frameborder="0" scrolling="yes" style="width:100%;height:100%;"></iframe>
			    </div>
		     </s:if>
		       </div>
