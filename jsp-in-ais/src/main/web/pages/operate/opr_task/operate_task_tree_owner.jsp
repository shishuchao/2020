     ...
     Ext.onReady(function(){
       ...
       //-------------审计事项grid--开始------
       	...
       	var dataModelItem={url:'${pageContext.request.contextPath}/operate/taskExt/getOwnerTaskListJson.action',
             cells:['taskTemplateId','taskMethod','taskPid','taskId','groupName','groupCode','taskNameView','taskName','taskCode','taskTarget','taskProgress','taskAssign','taskAssignBak','taskAssignName','project_id','id','cat_name','cat_code','taskMust','taskMustName','manu','doubt']
		};
		...
		var viewModelItem=[{header:'taskTemplateId',dataIndex:'taskTemplateId',hidden:true,sortable:true,dataIndex:'taskNameView',hidden:true,sortable:true},
		  ...
		  {header:'<div style="text-align:center">审计事项</div>',dataIndex:'taskName',width:240,renderer: formatQtip1},
		  {header:'<div style="text-align:center">审计小组</div>',dataIndex:'groupName',width:140,renderer: formatQtip2}
         ];	
         
         var gridItem =new Usp.Grid({
            gridConfig:{title:'',id:'common-data-datadetaillist-g2<%=owner%>',collapsible:true,autoHeight:true,autoWidth:true,collapsed:
            false,
	        ...
	        function rowdblclickFn(grid, rowindex, e){//单击事件   
               ...
               var u = '${pageContext.request.contextPath}/operate/task/project/showContentTypeView.action?groupCode='+groupCode+'&coll=0&taskTemplateId='+taskTemplateId+'&taskId='+taskId+'&taskPid='+taskPid+'&permission=<%=request.getParameter("permission")%>&isView=<%=request.getParameter("isView")%>&project_id=<%=request.getParameter("project_id")%>&owner='+<%=ownerStr%>
               if(taskPid=='-1'||typeof taskPid=='undefined'){ 
	               taskPid='-1';
	               project_id = '<%=request.getParameter("project_id")%>';
	               Usp.doTabLoad({
       ...
       function formatQtip1(data,cellmeta,record,rowIndex,columnIndex,color_store){ 
    	...
    	var tip =data; 
    	//alert(record.get('taskNameView'));
    	cellmeta.attr = 'ext:qtitle="' + title + '"' + ' ext:qtip="' + record.get('groupName') + '"';  
    	return data;  
   	   }
