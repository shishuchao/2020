...
$(function(){
    ...
    if(!isView){
        var offsetYear = 10;
        var yearArr = [];
        for(var i=2018; i<parseInt(currentYear)+offsetYear; i++){
            yearArr.push({
                'code':i,
                'name':i
            });
    ...
    function saveYearPlan(){
        ...
        if(checkDeprecatedTerm() && aud$validRowsTj('detailTable')){
				...
				if(data){
					if(data.type == 'info'){
                        ...
                        setTimeout(function () {
                        	var num = Math.random();
                            if(data.isAdd){
                                window.location.href='${contextPath}/plan/3year/edit.action?num='+num+'&sucflag=1&activeTabId=${activeTabId}&crudId='+data.formId;
                            }else{
...
<body style='padding:0px;margin:0px;overflow:hidden;'  class='easyui-layout' border='0' fit='true'>
	...
	<div region='center' border='0' title="" split="true" id="layoutCenter">
		<form id="planForm" name="planForm" method="post">
			...
			<input type="hidden" id="activeTabId" name="activeTabId" value="${activeTabId}"/>
			...
			<!-- 项目计划 -->
			<jsp:include flush="true" page="/pages/plan/3year/showWorkPlanDetail.jsp" />
