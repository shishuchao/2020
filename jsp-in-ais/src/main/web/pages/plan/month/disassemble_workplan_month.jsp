...
<html>
	...
	<s:tabbedPanel id="monthPlanPanel" theme='ajax'  selectedTab="${param.selectedTab}">
		...
		<s:div id='monthDetailListDiv' label='预选项目信息' theme='ajax'>
			<s:form id="monthDetailPlanSearchForm" action="disassemble" namespace="/plan/month" >
						...
						<td align="right" class="listtabletr1" colspan="4">
							<DIV align="right">
								...
								<input type="button" value="重置"
									onclick="window.location.href='${pageContext.request.contextPath}/plan/month/disassemble.action?crudId=${crudId}&&taskInstanceId=${taskInstanceId}&&selectedTab=monthDetailListDiv'">
							</DIV>
						</td>
					</tr>
				</table>
				<div align="center">
					<display:table name="monthDetailList" id="row" class="its" cellpadding="1" pagesize="10" 
							...
							requestURI="${contextPath}/plan/month/disassemble.action?selectedTab=monthDetailListDiv" >
						<display:column title="选择" headerClass="center" class="center">
							...
							<s:else>
								<input type="checkbox" name="detail_plan_id" disabled="true" value="${row.formId}"  onclick="changeButtonState(this,'${row.status_name}')" >
							</s:else>
						...
						<display:column title="计划编号" headerClass="center" sortable="true" sortProperty="w_plan_code">
							<a href="/ais/plan/month/viewMonthDetail.action?monthDetailId=${row.formId}" target="_blank">${row.w_plan_code}</a>
						</display:column>
					...
					<div align="right" style="width: 97%">
						...
						<s:if test="${crudObject.status_name=='正在执行'}">
							<input id="disassembleButton" type="button" value="确认立项" disabled="disabled" onclick="return disassembleMonth()"/>
						</s:if>
						&nbsp;&nbsp;
						<input type="button" value="返回" onclick="window.location.href='${contextPath}/plan/month/listAll.action'" />
	...
	<script type="text/javascript">
		/*
			立项月度计划
		*/
		function disassembleMonth(){
			var ids = document.getElementsByName("detail_plan_id");
			var detailIds = '';
			for(var i=0;i<ids.length;i++){
				if(ids[i].checked==true){
					window.location.href="/ais/plan/detail/edit.action?monthDetailId="+ids[i].value;
					break;
				}
			}
		}
