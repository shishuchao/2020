...
<html>
            ...
            $(function(){
            	...
            	if(fromAdjust == null || fromAdjust == '') {
            		if(w_plan_type == null || w_plan_type == '') {
            			$('#w_plan_type').val('020101');
            			$('#w_plan_type_name').val('年度计划');
            		}
            	} else {
            		if(w_plan_type == null || w_plan_type == '') {
            			$('#w_plan_type').val('020102');
            			$('#w_plan_type_name').val('临时计划');
            		}
            	}
            });
        </script>
		...
		<table cellpadding=0 cellspacing=0 border=0 align="center" class="ListTable" style="width: 98%;position: fixed;margin-top: 0px">
			<tr >
				<td align="left" class="EditHead">
					<div style="float:right"><s:hidden name="isStart" value="${isStart}"/>
						<s:if test="${!(defer||adjust||cancel||carryforward)}">
							...
							<s:if test="defer||adjust||cancel||carryforward">
								...
								<s:else>
									<a class="easyui-linkbutton" data-options="iconCls:'icon-undo'" onclick="this.style.disabled='disabled';window.location.href='${contextPath}/plan/detail/listAll.action?crudObject.w_plan_year=${crudObject.w_plan_year}&type=edit'" >返回</a>
								</s:else>
							</s:if>
							<s:else>
								<s:if test="${taskInstanceId!=null&&taskInstanceId>0}">
									<a class="easyui-linkbutton" data-options="iconCls:'icon-undo'" onclick="this.style.disabled='disabled';window.location.href='${contextPath}/bpm/taskinstance/pending.action?processName=${processName}&project_name=${project_name}&formNameDetail=${formNameDetail}'" >返回</a>
								</s:if>
								<s:else>
									<a class="easyui-linkbutton" data-options="iconCls:'icon-undo'" onclick="this.style.disabled='disabled';window.location.href='${contextPath}/plan/detail/listDetailStart.action?yearFormId=${yearFormId}&type=edit'" >返回</a>
								</s:else>
							</s:else>
					</div>
				</td>
	...
	<div style="overflow-y:hidden;padding-top: 30px" region="center">
		<s:form id="planForm" action="submit" namespace="/plan/detail">
			...
			<s:hidden name="crudObject.proSource"></s:hidden>
			<table id="planTable" name="planTable"  cellpadding=0 cellspacing=0 border=0 align="center" class="ListTable" style="width: 98%;">
				<tr>
					...
					<td class="editTd" style="width:35%">
						<%-- <s:textfield id="w_plan_code" name="crudObject.w_plan_code" cssClass="noborder"
								readonly="!(varMap['w_plan_codeWrite']==null?true:varMap['w_plan_codeWrite'])"
								display="${varMap['w_plan_codeRead']}"  title="计划编号" maxlength="25"/>
				...
				<tr>
					...
					<td class="editTd" style="width:35%" id="auditObjtContTd" colspan="3">
						<s:textfield cssClass="noborder" name="crudObject.audit_object_name"  id="audit_object_name" cssStyle="width:67%" readonly="true" maxlength="100"/>
				...
				<tr id="jjzrrTr"  style="display: none;">
					...
					<td class="editTd" style="width:35%" colspan="3">
						<s:textfield cssClass="noborder" name="crudObject.jjzrrname"  id="jjzrrName" cssStyle="width:67%" readonly="true" maxlength="100"/>
						<s:hidden name="crudObject.jjzrrid" id="jjzrr"></s:hidden>
					</td>
				</tr>
				<tr>
					<td class="EditHead" style="width:15%">
					...
					<td class="editTd" style="width:35%" colspan="3">
						<s:textfield name="crudObject.w_plan_name"  id="w_plan_name" cssClass="noborder"
									 readonly="true"
									 maxlength="255" cssStyle="width: 67%"/>
				...
				<tr>
					...
					<td class="editTd" style="width:35%">
						<s:textfield id="w_plan_type_name" name="crudObject.w_plan_type_name" cssClass="noborder" readonly="true"></s:textfield>
				...
				<tr>
					<s:if test="${isSyType == 'Y'}">
						...
						<td class="editTd" style="width:35%" id="planProcessTd03">
							<input id="xmTypeCombobox" name="xmTypeCombobox">
							<input type="hidden" id="xmType" name="crudObject.xmType" value="${crudObject.xmType}">
						</td>
					</s:if>
					<s:elseif test="${isSyType == 'F' } ">
						<s:hidden name="crudObject.xmType" value="syOff"></s:hidden>
						<s:hidden name="crudObject.planProcess" value="standard" id="planProcess"></s:hidden>
					</s:elseif>
					<s:else>
						<s:hidden name="crudObject.xmType" value="bs"></s:hidden>
					</s:else>

					<s:if test="${isSyType != 'F' } ">
						<s:if test="${planProcessConfig eq 'simple'}">
							<s:hidden name="crudObject.planProcess" value="simplified" id="planProcess"></s:hidden>
						</s:if>
						<s:elseif test="${planProcessConfig eq 'complete'}">
							<s:hidden name="crudObject.planProcess" value="standard" id="planProcess"></s:hidden>
						</s:elseif>
						<s:elseif test="${planProcessConfig == 'both-simple' || planProcessConfig == 'both-complete'}">
							<td class="EditHead" style="width:15%" id="planProcessTd01">
								<font color=red>*</font>&nbsp;审计流程
							</td>
							<s:if test="${isSyType == 'Y'}">
								<td class="editTd" style="width:35%" id="planProcessTd02">
									<input id="planProcessCombobox" name="planProcessCombobox">
									<input type="hidden" id="planProcess" name="crudObject.planProcess" value="${crudObject.planProcess}">
								</td>
							</s:if>
							<s:else>
								<td class="editTd" style="width:35%" id="planProcessTd02" colspan="3">
									<input id="planProcessCombobox" name="planProcessCombobox">
									<input type="hidden" id="planProcess" name="crudObject.planProcess" value="${crudObject.planProcess}">
								</td>
							</s:else>
				...
				<s:if test="${planProcessConfig != 'complete' && isSyType != 'F' }">
				<tr class="simplifiedEle">
					<td class="EditHead" style="width:15%">
						参考方案
					</td>
					<td class="editTd" style="width:35%" colspan="3">
						<s:buttonText2 id="audTemplateName" hiddenId="audTemplateId" cssClass="noborder"
									   name="crudObject.audTemplateName"
									   hiddenName="crudObject.audTemplateId"
									   doubleOnclick="showAudTemplateTree(this);"
									   doubleSrc="${pageContext.request.contextPath}/easyui/1.4/themes/icons/search.png"
									   doubleCssStyle="cursor:pointer;border:0"
									   readonly="true"
									   maxlength="500" title="参考方案"/>
	...
	<script language="javascript">
	  	...
	  	$(function(){
			...
			var isSyType = '${isSyType}';
			var planProcessConfig = '${planProcessConfig}';
			if((planProcessConfig == 'both-simple' || planProcessConfig == 'both-complete') && isSyType != 'F') {
				$('#planProcessCombobox').combobox({
					...
					onLoadSuccess:function(data){
						if('${crudObject.planProcess}' == ''){
							...
							$('#planProcessCombobox').combobox('setValue','${crudObject.planProcess}');
						}
					},
			...
			if(isSyType == 'Y') {
				$('#xmTypeCombobox').combobox({
					url:'${contextPath}/pages/plan/detail/xmType.json',
					valueField:'pkey',
					textField:'ptext',
					editable:false,
					panelHeight:60,
					onLoadSuccess:function(data){
						if('${crudObject.xmType}' == ''){
							$('#xmTypeCombobox').combobox('setValue',data[0].pkey);
							$('#xmType').val(data[0].pkey);
						} else {
							if(isSyType == 'F') {
								$('#xmTypeCombobox').combobox('setValue','standard');
							} else {
								$('#xmTypeCombobox').combobox('setValue','${crudObject.xmType}');
							}
						}
					},
					onChange:function(newVal,oldVal){
						$("#xmType").val(newVal);
						if(newVal == 'syOff') {
							if(planProcessConfig == 'both-simple' || planProcessConfig == 'both-complete') {
								$('#planProcessCombobox').combobox('setValue','standard');
							} else {
								$('#planProcess').val('standard');
							}

							$('#planProcessTd01').hide();
							$('#planProcessTd02').hide();
							$('#planProcessTd03').attr('colSpan',3);
							$('.simplifiedEle').hide();
							$('#audTemplateId').val('');
							$('#audTemplateName').val('');
						} else {
							if(planProcessConfig == 'both-simple' || planProcessConfig == 'both-complete') {
								...
								$('#planProcessTd01').show();
								$('#planProcessTd02').show();
								$('#planProcessTd03').attr('colSpan',1);
							} else {
								$('#planProcessTd03').attr('colSpan',3);
								if(planProcessConfig == 'simple') {
									$('#planProcess').val('simple');
									$('.simplifiedEle').show();
								} else if(planProcessConfig == 'complete') {
									$('#planProcess').val('standard');
								}
							}
						}
	...
	function validateDate2(){
		...
		*/
		function toSubmit(option){
			...
			var proSource = $("#proSource").val(); //项目来源
			var planProcess = $("#planProcess").val();//审计流程
/*			if(proSource !=null && planProcess != null){
				if(proSource == "inner" && planProcess == "simplified"){
					showMessage1("只有外部项目可以使用简易流程，内部项目不可使用简易流程!");
					return false;
				}
			}*/
...
/*			if(proSource !=null && planProcess != null){
			...
			if(frmCheck(flowForm,tableId)==false){
				...
				flowForm.action="${contextPath}/plan/detail/save.action?fromAdjust=${fromAdjust}";
				flowForm.submit();
			}
		}
		/**
		...
		*/
		...
		function showAudTemplateTree(target){
		    ...
		    var proChildType = $('#pro_type_child').val();
