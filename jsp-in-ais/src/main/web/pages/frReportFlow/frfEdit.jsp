...
$(function(){
    ...
    if('${taskInstanceId}' != '0'){
        reportUrl = frflow.getFrViewTemplateUrl(reportUrl);
    }
        ...
        toolbar:[{
             ...
             handler:function(){
				if(isLastFlowNode()){
                    ...
                    $('#reportContent').val(paramData.tableData);
				}
...
// 是否为流程的最后一个节点
function isLastFlowNode(){
	try{
       	 var optionValue = $("select[name='formInfo.toNodeId']").val();
    	 var endNodeNames = [];
    	 endNodeNames.push('<s:property value="@ais.bpm.util.BpmConstants@END_NAME"/>');
    	 endNodeNames.push('<s:property value="@ais.bpm.util.BpmConstants@END_NAME2"/>');
    	 if(optionValue && endNodeNames.length > 0){
    		 for(var i=0; i<endNodeNames.length; i++){
    			 if(optionValue.indexOf(endNodeNames[i]) != -1){
    				 return true;
    			 }
    		 }
    	 }
...
<s:else>
	...
	<div data-options="region:'center',title:'<span style=\'float:left\'>${crudObject.templateName}</span>',split:true" >
        <!-- 报表显示 -->
        <iframe id="showReportIfr" name="showReportIfr"  onload="frflow.reportReadOnly(this.id)" src=""  width="100%" height="100%" frameborder="0">
        </iframe>
        <!-- end -->
	</div>
