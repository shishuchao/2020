...
<head>
	...
	var problemNumberTables = function(){ 
		return {
			problemNumberChart:function(){
             ...
             /* 变换统计维度 */
             	...
             	var echart = echarts.init(document.getElementById('problemamount'));
	        	echart.showLoading({
	                text:'数据加载中，请稍后...'
	            }); 
                ...
                var legend = [];//echarts的legend
               	var tableList;
      			var average;
    			$.getJSON('<%=request.getContextPath()%>/proledger/problem/rankingAuditQuestions1.action',
    					{'pro_year':pro_year,'audit_dept':audit_dept,'audit_dept_name':audit_dept_name,'audit_object':audit_object,
    					 'audit_object_name':audit_object_name,'topN':topN,'sort_big_code':sort_big_code,'sort_big_name':sort_big_name,
    					 'pro_type':pro_type,'mend_state':mend_state,'type':type,'sort':sort},
    					 function(json){
    	        	    legend = json.legend;
	            ...
	            if(sort!=""&&sort!="null"){	           
	      	   ...
	      	   if(dataList && legend && legend.length>0){   
	        	   var options = {  
	      	 	 ...
	      	 	 $("#problemamount").empty().append('<h3 style="text-align:center;">暂时没有信息</h3>');
	      	 };
		...
		$(document).ready(function(){
			problemNumberTables.problemNumberChart();	
		});
	</script>
</head>
<body >
	<div id="chartbody">
		<div> 			
			<div id="problemamount" class="chartcss"></div>
		</div>
	    <span id='Average' name='Average' class="spanstb"></span> 
		<div >
	    	<table id="auditwtTable" class="auditwtTable" cellspacing="0" cellpadding="0"   >
		      	<thead>
		           <tr class="datagrid-header datagrid-header-row datagrid-row">
		               <td  class="tbTitle5">序号</td>
		               <td  id='title' class="tbTitle10">审计单位</td>
		               <td  class="tbTitle5">问题数量</td>
		               <td  class="tbTitle8">涉及金额(万元)</td>
		               <td  class="tbTitle8">查出违规违纪金额(万元)</td>
		               <td  class="tbTitle8">已纠正违规违纪金额(万元)</td>
		           </tr>
		      	</thead>
	      		<tbody>
	       		</tbody>
