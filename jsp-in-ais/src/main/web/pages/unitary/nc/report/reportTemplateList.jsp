...
<script type="text/javascript">
	$(function(){
		...
		$('#templateList').datagrid({
			url : "<%=request.getContextPath()%>/unitary/nc/autoreport/reportTemplateList.action?template.projectTypeCode=<%=request.getParameter("projectTypeCode")%>&fromType=<%=request.getParameter("fromType")%>",
			method:'post',
			showFooter:false,
			rownumbers:true,
			striped:true,
	...
	function modifyTemplate(templateId,projectTypeCode){
		var url = "<%=request.getContextPath()%>/unitary/nc/autoreport/reportData.action?templateId="+templateId+"&projectTypeCode="+projectTypeCode+"&fromType=<%=request.getParameter("fromType")%>";
