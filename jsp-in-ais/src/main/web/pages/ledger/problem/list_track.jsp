...
<html>
	...
	<div style="margin: 0;padding: 0;overflow:hidden;" class="easyui-layout" fit="true">
		<div id="problemTracking_div" title="整改结果跟踪评价记录" style='overflow:hidden;padding:0px;'> 	  		
			<form id='problemTracking_form' name='problemTracking_form' method="POST" style='height:90%;overflow-y:auto;overflow-x:hidden;padding:10px;margin:0px;border:1px solid #cccccc;' >
				<table class="ListTable" cellpadding=1 cellspacing=1 id="tab4" align="center" style="width:98%">
					...
					<tr>
						...
						<td class="editTd">
							<%--<input type="text" class="noborder money editElement" id="increase_money" name="problemTracking.increase_money" value="${problemTracking.increase_money}" maxlength="20"/>--%>
							<s:textfield cssClass="noborder money editElement" id="increase_money" name="problemTracking.increase_money" value="%{formatMoney(problemTracking.increase_money)}"></s:textfield>
						</td>
					</tr>
					<tr>
						...
						<td class="editTd">
							<%--<input type="text" class="noborder money editElement" id="loss_money" name="problemTracking.loss_money" value="${problemTracking.loss_money}" maxlength="20"/>--%>
							<s:textfield cssClass="noborder money editElement" id="loss_money" name="problemTracking.loss_money" value="%{formatMoney(problemTracking.loss_money)}"></s:textfield>
						</td>
					</tr>
			    </table>
	    ...
	    <script type="text/javascript">
		    $(function(){
		    	...
		    	$('#trackingList').datagrid({
					url : "<%=request.getContextPath()%>/easyui/problem/trackList.action?querySource=grid&crudIdStrings=${crudIdStrings}&onlyView=true&problemcode=${problemcode}",
					method:'post',
					showFooter:false,
					rownumbers:true,
					pagination:true,
							...
							handler:function(){
								$.ajax({
									...
									success: function(data){
										...
										$('#fj_div').fileUpload('property', 'fileGuid', data.uuidString);
										$('#fj_div').fileUpload('setGridBtn',[{id:'add',display:true},{id:'edit',display:true},{id:'view',display:false},{id:'delete',display:true},{id:'download', display:true}]);
										$('#fj_div').fileUpload('reloadFile');
//										$('#fj_div').fileUpload({
//											fileGuid:data.uuidString
//										});
									},
									error:function(data){
										showMessage1('请求失败！');
						...
						}, */
						{
							...
							handler:function(){
								...
								if(rows.length==1){
									...
									var middleLedgerProblem_id = rows[0].middleLedgerProblem_id;
									var flag = true;
									$.ajax({
										type: "POST",
										url: "${contextPath}/proledger/problem/getProblemTrackingById!beforeProblemTracking.action",
									...
									if(flag){
										trackUrl = "${contextPath}/proledger/problem/editAudTrackingLedgerEvaluate.action?middleLedgerProblem_id="+middleLedgerProblem_id+"&project_id=${project_id}&isEdit=isEdit&id="+id;
								...
								if(rows.length>0){
									$.messager.confirm('确认','确定删除么？', function(r){
										if (r) {
											...
											$.ajax({
												type: "POST",
												url: "${contextPath}/proledger/problem/getProblemTrackingById!beforedeleteProblemTracking.action",
												data:{'crudIdStrings':crudIdStrings},
												async: false,
												success: function(returnValue){
													var json=eval("("+returnValue+")");
													if(json[0]=='in'){
														showMessage2('来自项目内整改问题跟踪记录不能删除!');
														flag = false;
													}else if(json[1]=='yes'){
														showMessage2('不是本人记录不能删除!');
														flag = false;
													}
												}
											});
											if(flag){
												$.ajax({
													...
													success: function(data){
														...
														window.location.href="${contextPath}/proledger/problem/trackList.action?crudIdStrings="+crudIdStrings;
													},
													error:function(data){
														showMessage2('请求失败！');
													}
											...
											if(data.type === 'success'){
												...
												for ( var p in rowData ){
													...
													if (p == 'examine_result'){//标签是< textarea
														...
														$('#feedback_date').html(value); 
													}else if(p == 'tracker_code'){ 
														$('#'+p+'temp').val(value); 
													}else if(p == 'tracker_name'){
														$('#'+p+'temp').val(value); 
														$('#'+p).val(value);
													}else if(p == 'f_mend_accessory'){// 附件
														...
														$('#'+p).val(value);
													}else if (p == 'increase_type_code'){
														$('#'+p).combobox('setValue', value);
													}else if (p == 'loss_type_code'){
														$('#'+p).combobox('setValue', value);
													}else if (p == 'increase_money'){
														$('#'+p).val(value);
													}else if (p == 'loss_money'){
														$('#'+p).val(value);
													};
												}
												$("#trackBtnDiv").hide();
					...
					frozenColumns:[[
						<s:if test="${onlyView!='true'}">
						{field:'id',width:0.1*bodyW+'px', checkbox:true, align:'center',sortable:true},
						</s:if>
						{field:'serial_num',title:'问题编号',halign:'center',align:'left',sortable:true,formatter:function(value,row,rowIndex){
						...
						{field:'mend_result',title:'被审计单位反馈整改结果',width:0.2*bodyW+'px',sortable:true,halign:'center',align:'left'}
					]],
					columns:[[  
				...
				$('#saveProblemTracking').bind('click',function(){
					...
					$.ajax({
						dataType:'json',
						url : "${contextPath}/proledger/problem/saveProblemTracking.action?crudIdStrings="+crudIdStrings+"&flag=1",
			...
			function back(){
				var url='<%=request.getContextPath()%>/proledger/problem/trackProblemList.action';
