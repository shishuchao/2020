...
2020.2.27，qfucee
1.增加如下方法，方便topDialog调用多层嵌套在父页面里面的iframe
//重新设置页面的topDialogTargetId
function resetTopDialogTargetId(){}
//外层窗口要调用的iframe对象(dom)
function aud$calledIfm(type){}
//topDialog窗口call父页面时，调用此方法
function aud$TopDialogCallback(callbackFn, type){}
//topDialog调用父页面或者父页面里面子页面或者子页面的子页面等
function aud$TopDialogCallParent(callbackFn, type){}
*/
...
function aud$getTabIframByTabId(tabId){
    try{
        if(tabId){
            ...
            */
        	var ifmWin = top.$('#'+tabId+'id').get(0).contentWindow;
        	return ifmWin;
        }
    }catch(e){
...
function aud$getActiveTab(){
    try{
    	...
    	return top.$('#tabs').tabs('getSelected');
    }catch(e){
...
function aud$getActiveTabTitle(tabId){
	try{
		var tabId = tabId ? tabId : aud$getActiveTabId();
		return top.$('#'+tabId).parents('.easyui-tabs').find('li.tabs-selected .tabs-title').text();
	}catch(e){
		alert('aud$getActiveTabTitle:'+e.message);
	}
}
...
function aud$closeTab(tabId, parentTabId){
    try{
        ...
        if(topDialogTargetId){
        	...
        	top.$('#tabs').tabs('close',aud$getActiveTabTitle());
        }
...
function aud$activeTab(tabId){
    try{
    	...
    	var title = aud$getActiveTabTitle(tabId);
...
function aud$openNewTab(title, url, isDialog, isShowProgress){
    ...
    if(isDialog){//使用最外层窗口
        ...
        top.addNewTab(title ? title: '新标签',url,tabId, null);
        return tabId;
    }
...
function aud$createTopDialog(options){
    try{
        ...
        var draggable = options.draggable != null ? options.draggable : true;
        // alert('topWin='+topWin)
...
function audRecomputeHomeStBar(homeStBarId, divcount, divwidth){
	/*
	setTimeout(function(){
		divcount = divcount ? divcount : 5;
		var totalDivWidth = divcount * (divwidth ? divwidth : 240);
		var divSplitWidth = Math.floor($('#'+ homeStBarId).width()) - totalDivWidth;
		$('.splitbar').css('margin-left', Math.round(divSplitWidth/divcount));
		if(divwidth){
			$('.groupPadding').css('width', divwidth);
		}
	}, 0)
	*/
}
...
function resetTopDialogTargetId(){
	$(function(){		
		//把iframe的外的页面的topDialogTargetId设置为iframe的parentDialogId
		setTimeout(function() {
			var topDialogTargetId = parent.$('body').attr("topDialogTargetId");
			//alert('topDialogTargetId:'+topDialogTargetId)
...
function aud$TopDialogCallback(callbackFn, type){
	try{
		var targetIfm = aud$calledIfm(type);
		//alert("targetIfm:"+targetIfm)
		if(targetIfm){
			var subWin = targetIfm.contentWindow;
			//alert('subWin:'+subWin)
			subWin.aud$TopDialogCallback(callbackFn);
		}else{
			callbackFn ? callbackFn(window) : null;
		}
	}catch(e){
		alert("aud$TopDialogCallback:\n"+e.message);
	}
}
...
function aud$TopDialogCallParent(callbackFn, type){
    try{
        ...
        //alert(tabWin)
        if(tabWin && tabWin.aud$TopDialogCallback){
           tabWin.aud$TopDialogCallback(callbackFn, type);
        }
    }catch(e){
    	alert("aud$TopDialogCallParent:\n" + e.message)
    }
}
