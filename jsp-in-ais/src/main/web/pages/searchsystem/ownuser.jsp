...
<display:table name="userlist" id="row" class="its" pagesize="10" requestURI="${pageContext.request.contextPath}/systemnew/getUserOwnList.action?p_issel=${p_issel}&p_item=${p_item}&p_deptid=${p_deptid}&p_deptname='${p_deptname}'" excludedParams="*">
			<display:column title="选择"  style="width:30" headerClass="center" class="center">
			<s:if test="${not empty (requestScope.p_deptname)}">
			<input type="checkbox" id="node${row_rowNum}" name="checkvalue" isnode="true" loginname="${row.floginname}" value="${row.fuserid},${row.floginname},${row.fname},${requestScope.p_deptname},${p_deptid}"/>
			</s:if><s:else>
			<input type="checkbox" id="node${row_rowNum}" name="checkvalue" isnode="true" loginname="${row.floginname}" value="${row.fuserid},${row.floginname},${row.fname},${row.fdepname},${p_deptid}"/>
			</s:else>
    		</display:column> 
