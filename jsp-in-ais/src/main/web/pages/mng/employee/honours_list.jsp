...
<div id='queryCond6'>
    <select id="query_honornameCode" name="query_honornameCode"></select>
</div>
...
<script type="text/javascript">
        ...
        if (rows && rows.length) {
            ...
            for (var i = 0; i < rlen; i++) {
                ...
                if (!row.gettime || row.gettime.replace(/\s+/g, "").length == 0) {
                    ...
                    showMessage1("第"+(i+1)+"行[获得时间]不能为空");
                    return false;
                }
                if (!row.honours_name_code || row.honours_name_code.replace(/\s+/g, "").length == 0) {
                    editFlag = i;
                    $('#sDTable').datagrid('selectRow', editFlag).datagrid('beginEdit', editFlag);
                    showMessage1("第"+(i+1)+"行[荣誉名称]不能为空");
                    return false;
                }

                //荣誉名称选其他
                if (row.honours_name == "其他") {
                    if (!row.remarks || row.remarks.replace(/\s+/g, "").length == 0) {
                        editFlag = i;
                        $('#sDTable').datagrid('selectRow', editFlag).datagrid('beginEdit', editFlag);
                        showMessage1("第"+(i+1)+"行[荣誉名称]选择其他时，[备注说明]必填！");
                        return false;
                    }
                }
            }
    ...
    $(function () {
        ...
        var honorNameArr = eval('${honorNameArr}');

        frloadOpen();

        aud$genSelectOption('query_honornameCode', honorNameArr);
        ...
        var g1 = new createQDatagrid({
            ...
            gridJson: {
                ...
                toolbar: [
                    ...
                    , '-', {
                        ...
                        handler: function () {
                            ...
                            $.ajax({
                                url: '${contextPath}/mng/employee/personnel/saveHonours.action',
                                async: false,
                                type: 'POST',
                                data: {'rowstr': rowstr},
                ...
                columns: [[
                    ...
                    {
                        field: 'gettime', title: '获得时间', width: bodyW * 0.15 + 'px', align: 'center', halign: 'center', sortable: true,
                        editor: {
                            type: 'datebox',
                            options: {
                                editable: false,
                    ...
                    {
                        field: 'honours_name_code', title: '荣誉名称', width: bodyW * 0.15 + 'px', align: 'center', halign: 'center', sortable: true, type: 'custom', target: $('#queryCond6')[0], show: 'true',
                        editor: {
                            type: 'combobox',
                            options: {
                                data: honorNameArr,
                        ...
                        formatter: function (value, row, index) {
                            var vtext = "";
                            $.each(honorNameArr, function (i, jdata) {
                                if (jdata['code'] == value) {
                                    vtext = jdata['name'];
                                    return false;
                                }
                            ...
                            row['honours_name'] = rtVal;
                            return rtVal;
                        }
                    },
                    {
                        field: 'remarks', title: '备注说明', width: bodyW * 0.25 + 'px', align: 'left', halign: 'center', sortable: true,
                        editor: {//设置其为可编辑
                            type: 'text',
                            options: {}
                        }
                    ...
                    {
                        field: 'issuing_unit', title: '颁发单位', width: bodyW * 0.15 + 'px', align: 'center', halign: 'center', sortable: true,
                        editor: {//设置其为可编辑
                            type: 'text'
                        }
                    },
                    {
                        field: 'file_id', title: '证明材料', width: bodyW * 0.25 + 'px', align: 'center', halign: 'center', sortable: true,
