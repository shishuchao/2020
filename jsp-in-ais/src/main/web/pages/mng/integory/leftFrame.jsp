...
<script type="text/javascript">
	...
	Ext.QuickTips.init();
	 ...
	 <s:iterator value="orglist">
	 	<s:if test="${fpid=='0'}">
        var root=new Tree.TreeNode({id:'${fid}',text:'<s:property value="fname"/>',href:'<%=request.getContextPath()%>/resmngt/integory/searchIntegory.action?auditunitid=${fid}&flag=${flag }',hrefTarget:'childBasefrm',draggable:false //不允许拖拽
      ...
      <s:else>
      	<s:if test="${!isHaveChild}">
      		<s:if test="${orgType=='1'}">
      			var node_${treefid}=new Tree.TreeNode({id:'${fid}',qtip:'${fname}',text:'${fname}',href:'<%=request.getContextPath()%>/resmngt/integory/searchIntegory.action?auditunitid=${fid}&flag=${flag }',leaf:false,hrefTarget:'childBasefrm'});
      		</s:if>
      		<s:else>
