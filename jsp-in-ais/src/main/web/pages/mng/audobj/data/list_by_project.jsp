...
<body style="margin: 0;padding: 0;overflow:hidden;" class="easyui-layout" border='0'>
	<div id="dlgSearch" class="searchWindow">
		<div class="search_head">
			<s:form namespace="/mng/audobj/data" action="listProjectAuditObjectData" id="searchForm">
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$('#audDataGrid').datagrid({
				url : "<%=request.getContextPath()%>/mng/audobj/data/listAuditObjectDataByProject.action?querySource=grid&projectId=${projectId}",
				method:'post',
				...
				columns:[[
					...
					{field:'uploadTime', title:'上传时间', halign:'center', align:'center', width:bodyW * 0.15 + 'px', sortable:false}
				]]
			});
				...
				buttons:[
					{
						...
						handler:function(){
							...
							$("#uploadFile_form").form('submit',{
								url:'${contextPath}/mng/audobj/data/uploadFile.action?projectId=${projectId}',
								onSubmit:function(){
									try{
										aud$loadOpen();
		...
		function assignRow() {
			...
			if (rows.length == 0) {
				...
				for (var i = 0; i < rows.length; i++) {
					...
					<s:if test="${!isLeader}">
					if (rows[i].uploader != '${user.floginname}') {
						showMessage1('组员只能操作本人上传的数据！');
						return false;
					}
					</s:if>
				}
