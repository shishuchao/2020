...
<body style="margin: 0;padding: 0;overflow:hidden;" class="easyui-layout" fit='true' border='0'>
	...
	<br>
		<%-- 
		<a href="<%=request.getContextPath()%>/log/indexBackUpLog.action">indexBackUpLog 手工备份</a>		
		<a href="<%=request.getContextPath()%>/log/listBackUpLog.action">listBackUpLog 备份文件管理</a>		
		<a href="<%=request.getContextPath()%>/log/indexLoadLog.action?onLine=onLine">indexLoadLog 离线日志管理</a>		
		--%>
	<br>
	<script type="text/javascript">
		...
		function doView(logId){
		  window.location.href="/ais/log/view.action?logId="+logId+"&onLine="+$("#onLine").val();
		}		
		$(function(){
			// 初始化生成表格
			$('#its').datagrid({
