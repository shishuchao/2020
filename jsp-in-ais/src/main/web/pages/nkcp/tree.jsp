...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			Ext.onReady(function(){
		    	...
		    	var loader=new Ext.tree.TreeLoader({url:'${contextPath}/nkcp/asyCategoryTree.action?categoryId='+root.id});
				treePanel=new Tree.TreePanel({
		     	...
		     	treePanel.on('beforeload',function(node){
                	treePanel.loader.url='${contextPath}/nkcp/asyCategoryTree.action?categoryId='+node.attributes.id;
                });
		     	treePanel.on('beforeclick',function(node,e){
	                if(typeof node.attributes.href=='undefined'){
	                	parent.childBasefrm.location.href='${contextPath}/nkcp/tabpanel.action?categoryId='+node.attributes.id;
	                };
                });
	...
	<body>
		...
		<br />
		<div id="buttonDiv">
			<button onclick="addCategory()">新增内控节点</button>
			<br/>
			<button onclick="modifyCategory()">修改内控节点</button>
			<br/>
			<button onclick="deleteCategory()">删除内控节点</button>
		</div>
		<s:form id="categoryForm" action="saveCategory" namespace="/nkcp" >
			<s:hidden id="id" name="rootCategory.id" />
			<s:hidden id="name" name="rootCategory.name" />
			<s:hidden id="pid" name="rootCategory.pid" />
			<s:hidden id="pid" name="rootCategory.projectFormId" />
		</s:form>
		<script type="text/javascript">
			/*
			*	新增
			*/	
			function addCategory(){
				...
				var newName = window.showModalDialog('${contextPath}/pages/nkcp/edit_category.jsp',window,'dialogWidth:500px;dialogHeight:300px;status:yes');
			...
			*/	
			function modifyCategory(){
				...
				var newName = window.showModalDialog('${contextPath}/pages/nkcp/edit_category.jsp',window,'dialogWidth:700px;dialogHeight:450px;status:yes');
				if(newName==null || newName==''){
					return ;
				}
				document.getElementById('name').value = newName;
			...
			*/	
			function deleteCategory(){
				...
				if(confirm('确认要删除内控节点吗？执行此操作后节点和它所有的下级节点以及节点的调查项与测试项都将会被删除！')){
					...
					window.location.href="/ais/nkcp/deleteCategory.action?categoryId="+categoryId;
				}
			}
		</script>
	</body>
