			...
			*/
			...
			$(function(){
				...
				datagrid = $('#projectList').datagrid({
				    url:"<%=request.getContextPath()%>/listUserOnlineTime.action?querySource=grid",
					method:'post',
					showFooter:false,
					rownumbers:true,
					pagination:false,
			...
			function viewDetail(userId) {
				var url = "${contextPath}/listOneUserOnlineTime.action?floginname=" + userId + "&startTime=" + $("#startTime").datebox('getValue') + "&endTime=" + $("#endTime").datebox('getValue');
				aud$openNewTab(userId + "在线时间详情",url,false);
			}
 		</script>
	</body>
</html>
