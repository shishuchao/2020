		...
		<script type="text/javascript">
			...
			*/
			function riskEvaluateSure(id){
				var url = "${contextPath}/riskManagement/management/riskEvaluate/riskEvaluateSureTaskDetail.action?id="+id;
				aud$openNewTab('确认审核', url);
			}
			
			function viewRiskEvaluateSure(id) {
				var url = "${contextPath}/riskManagement/management/riskEvaluate/riskEvaluateSureTaskDetail.action?view=Y&id="+id;
				aud$openNewTab('查看审核', url);
			}
			...
			$(function(){
			    // 初始化生成表格
				$('#resultList').datagrid({
					url : "<%=request.getContextPath()%>/riskManagement/management/riskEvaluate/listRiskEvaluatesSure.action?querySource=grid",
					method:'post',
					showFooter:false,
					rownumbers:true,
					pagination:true,
					...
					columns:[[  
						...
						{field:'operate',
							 ...
							 formatter:function(value,row,index){
								 if(row.status == '2')
								 	return '<a href=\"javascript:void(0)\" onclick=\"riskEvaluateSure(\''+row.formId+'\',\''+row.status+'\');\">确认审核</a>';
								 else
								 	return '<a href=\"javascript:void(0)\" onclick=\"viewRiskEvaluateSure(\''+row.formId+'\',\''+row.status+'\');\">查看</a>';
							 }
						}
					]]   
				});
