...
<!DOCTYPE HTML>
<html>
	<title>风险评估结果确认审核--待评估风险列表</title>
	<head>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			g1 = new createQDatagrid({
				...
				gridJson:{    
					...
					columns:[[
						{field:'riskFatherSortName', title:'风险大类', width:bodyW*0.08 + 'px', align:'center', hidden:true},
						{field:'riskSonSortName', title:'风险子类', width:bodyW*0.08 + 'px', align:'center', hidden:true},
						{field:'id', title:'主键', hidden:true, align:'center', show:'false'}, 
						{field:'riskDescription', title:'风险描述', width:bodyW*0.2 + 'px', align:'center', sortable:true, show:'true', oper:'eq'},
						...
						{field:'operation', title:'操作', width:bodyW*0.1 + 'px', align:'center', sortable:false, show:'false',
							formatter:function(value,rowData,rowIndex){
								var url = '${contextPath}/riskManagement/management/riskEvaluate/listBasicInfo.action?riskEvaluate=${riskEvaluate}&id=' + rowData.id;
								if('${view}' == 'Y'){
									url += '&isView=Y';
									return '<a href=\'#\' onclick="evaluateSure(\'' + url + '\')">查看</a>';	 
								}else{
									var evaluated;
									$.ajax({
										url:'${contextPath}/riskManagement/management/riskImplement/isEvaluated.action',
										data:{'id':rowData.id,'all':'Y'},
										type:'POST',
										async:false,
									...
									if(rowData.status != '2') {
										if(evaluated == '1')
					            	   		 return '<a href=\'#\' onclick="evaluateSure(\'' + url + '\')">审核</a>';
					            	   	else {
					            	   		url += '&isView=Y';
											return '<a href=\'#\' onclick="evaluateSure(\'' + url + '\')">查看</a>';	 
					            	   }
		...
		function evaluateSure(url) {
			aud$openNewTab('审核',url, true);
		}
