...
<body id="body1" class='easyui-layout' fit='true' style='margin:0px;overflow:hidden' >
	<div region="center" border="0" style="overflow:hidden;" >
		<div class='easyui-tabs' border="0" style="overflow:hidden;width:100%;" fit='true'>
			...
			<div id='main' title='总体事项' <% if(StringUtils.isNotBlank(request.getParameter("selectedTab")) && request.getParameter("selectedTab").equals("main")){%> selected="true"<%} %> style="overflow:hidden;">
				<iframe id="basefrm1" name="baseifrm"
					src="${contextPath}/riskManagement/management/riskEvaluate/showContentRoot.action?node=${node}&path=${path}&selectedTab=main&taskId=<%=request.getParameter("taskId")%>&templateId=<%=request.getParameter("templateId")%>&riskevaluate_id=${riskevaluate_id}"
		    ...
		    <s:if test="'1' == '${type}'">
			    <div id='item' title='风险类别' <% if(StringUtils.isNotBlank(request.getParameter("selectedTab")) && request.getParameter("selectedTab").equals("item")){%> selected="true"<%} %> style="overflow:hidden;">
					<iframe id="basefrm2"
					src="${contextPath}/riskManagement/management/riskEvaluate/showContentType.action?node=<%=request.getParameter("node")%>&path=<%=request.getParameter("path")%>&selectedTab=item&type=<%=request.getParameter("type")%>&taskPid=<%=request.getParameter("taskPid")%>&taskId=<%=request.getParameter("taskId")%>&templateId=<%=request.getParameter("templateId")%>&riskevaluate_id=<%=request.getParameter("riskevaluate_id") %>&parentTabId=<%=request.getParameter("parentTabId") %>"
		    ...
		    <s:if test="'2' == '${type}'">
				<div id='item' title='风险事项' <% if(StringUtils.isNotBlank(request.getParameter("selectedTab")) && request.getParameter("selectedTab").equals("item")){%> selected="true"<%} %> style="overflow:hidden;">
					<iframe id="basefrm2"
					src="${contextPath}/riskManagement/management/riskEvaluate/showContentLeaf.action?node=<%=request.getParameter("node")%>&path=<%=request.getParameter("path")%>&selectedTab=item&type=<%=request.getParameter("type")%>&taskPid=<%=request.getParameter("taskPid")%>&taskId=<%=request.getParameter("taskId")%>&templateId=<%=request.getParameter("templateId")%>&riskevaluate_id=${riskevaluate_id}"
					frameborder="0" width="100%" height="100%"  ></iframe>
			    </div>
		    </s:if>
		</div>
