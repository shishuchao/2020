		    ...
		    showSysTree(taskTree,{
		        ...
		        onTreeClick:function(node, treeDom){
		            try{
		                ...
		                if(!taskPid){
		                	window.parent.frames['childBasefrm'].location.href="${contextPath}/riskManagement/management/riskEvaluate/showContent.action?node="+node.attributes.taskId+"&riskevaluate_id="+riskevaluate_id+"&parentTabId="+parentTabId+"&path="+path+"&selectedTab=main&type="+type+"&taskPid="+taskPid+"&taskId="+id+"&templateId=<%=request.getParameter("templateId")%>";
		            	}else{
		            		window.parent.frames['childBasefrm'].location.href="${contextPath}/riskManagement/management/riskEvaluate/showContent.action?node="+node.attributes.taskId+"&riskevaluate_id="+riskevaluate_id+"&parentTabId="+parentTabId+"&path="+path+"&selectedTab=item&type="+type+"&taskPid="+taskPid+"&taskId="+id+"&templateId=<%=request.getParameter("templateId")%>";
		            	}
		            }catch(e){
		...
		function locationTaskTreeNode(nodeId, text){
			try{
				...
				var timeout = window.setTimeout(function(){
					//alert('nodeId='+nodeId)
					if(nodeId){
						var url = "${contextPath}/commonPlug/getPriorNodeIdsById.action?rootParentId=-1&beanName=AudTaskTemplateTree&treeId=taskId&treeText=taskName&treeParentId=taskPid&treeOrder=taskOrder";
...
<body class='easyui-layout' fit='true' border="0">
	...
	<s:hidden id="parentTabId" value="${parentTabId}"></s:hidden>
	...
	<!-- <div region="south" style='padding:5px;overflow:hidden;text-align:right;white-space:nowrap;height:40px' border="0">
        <a href='javascript:;' id='addTaskBtn' class="easyui-linkbutton"  iconCls="icon-import">导入事项</a>
    </div> --> 
</body>
</html>
