...
$(function(){
    ...
    g1 = new createQDatagrid({
        ...
        pkName:'id',
        whereSql: 'status in (\'3\',\'4\')  and riskEvaluateTaskId in (select formId from RiskEvaluateTask where status=\'3\') and responsibilityDep=\''+fdepid+'\'',
        order:'desc',
        sort:'prioritySort',
        winWidth:800,
        winHeight:250,
        gridJson:{    
            ...
            frozenColumns:[[{field:'id', title:'选择', checkbox:false, align:'center', show:'false', hidden:true},
                            {field:'evaluateSerialNum',title:'编号', width:bodyW*0.1 + 'px',align:'center', sortable:true, show:'true', oper:'eq'},
                            {field:'evaluateYear',title:'年度', width:bodyW*0.05 + 'px',align:'center', sortable:true, type:'custom', target:$('#query_year')[0]},
                            {field:'affiliatedDeptName',title:'所属单位', width:bodyW*0.1 + 'px',align:'left', halign:'center', sortable:true, type:'custom', target:$('#queryCond1')[0]},
                            {field:'responsibilityDepName',title:'责任部门', width:bodyW*0.1 + 'px',align:'left', halign:'center',sortable:true, type:'custom', target:$('#queryCond2')[0]}
                            ]],
            columns:[[
			   {field:'riskFatherSortName',title:'风险大类', width:bodyW*0.1 + 'px',align:'center', sortable:true, show:'true', oper:'like', hidden:true},
			   {field:'riskSonSortName',title:'风险子类', width:bodyW*0.1 + 'px',align:'center', sortable:true, show:'true', oper:'like', hidden:true},
			   {field:'riskItemName',title:'风险事项', width:bodyW*0.1 + 'px',align:'left', halign:'center',sortable:true, show:'true', oper:'like'},
               ...
               {field:'operation',width:bodyW*0.08 + 'px',title:'操作', align:'center', sortable:false, show:'false', oper:'eq',
            	   formatter: function(value, rowData, rowIndex) {
            		   var url = '${contextPath}/riskManagement/management/riskHandle/edit.action?id=' + rowData.id;
            		   if(rowData.status == '3') {
            			   var handled;
            			   $.ajax({
            			   ...
            			   if(handled == '1'){
            				   ...
            				   return '<a href=\'#\' onclick="handle(\'' + url + '\')">应对</a>';
            			   }
            		   } else {
            	    	   url += '&isView=Y';
            	    	   return '<a href=\'#\' onclick="view(\'' + url + '\')">查看</a>';
