    			...
    			onSelect:function(title,id){
    				if(title == '应对方案') {
    					if($('#myFrame2').attr('src') == '') {
    						var url = '${contextPath}/riskManagement/management/riskHandle/editHandlePro.action?isView=Y&retId=${id}';
        					$('#myFrame2').attr('src',url);
    					}
    				}else if(title == '执行结果'){
    					if($('#myFrame3').attr('src') == '') {
    						var url = '${contextPath}/riskManagement/management/riskHandle/editImpResult.action?isView=Y&retId=${id}';
...
<body>
    <!-- <div class="easyui-panel" title="登记风险" fit='true' style="overflow: visible;"> -->
