...
<script type="text/javascript">
 ...
 $(function(){
			...
			onSelect : function(name, id) {
				if(name == '调整风险列表'){
					if($('#myFrame2').attr('src') == '') {
						$('#myFrame2').attr('src','${contextPath}/riskManagement/management/riskRegister/listRegisterAdjust.action?riskType=${riskCollectTask.riskType}&collectTaskName=${riskCollectTask.taskName}&collectTaskYear=${riskCollectTask.year}&collectTaskId=${crudObject.formId}');
					}
				}
			}
		}); 
	...
	function toSubmit(act){
        var value = document.getElementById('illustration').value;
        if (value == ''){
            showMessage1("说明不能为空！");
            return false;
        }else{
	...
	function viewRiskCollectTask() {
		var url = "${contextPath}/riskManagement/management/riskCollect/viewFdInfos.action?isTask=Y&formId=${crudObject.rctFormId}";
		aud$openNewTab('风险信息收集',url,true);
	}
	
	 function getTaskPid(obj,templateId) {
		 showSysTree(obj,
					{ reeTabTitle1:'风险树',
					  cascadeCheck:false,
					  dnd:true,
...
<s:else>
    ...
    <div class="easyui-layout" title="" fit='true' style="overflow: auto; width: 100%;height: 100%;">
    	...
    	<s:form id="myForm">
				...
				<table class="ListTable" align="center" style='width: 98.3%; padding: 0px; margin: 0px;'>
					<tr class="EditHead">
						...
						<span style='float: left; text-align: left;'>
							风险收集任务：<a href="javascript:void(0);" onclick="viewRiskCollectTask();">${crudObject.rctTaskName}</a>&nbsp;
						</span>
					   </td> 
						<td>
    		...
    		<s:hidden name="crudObject.taskName"></s:hidden>
    		...
    		<s:hidden name="crudObject.fdPerson"></s:hidden>
    		<s:hidden name="crudObject.fdPersonName"></s:hidden>
    		<s:hidden name="crudObject.fdTime"></s:hidden>
    		<s:hidden name="crudObject.rctTaskName"></s:hidden>
    		...
    		<table id='myTable' align="center" class="ListTable" style='margin-top: 40px; overflow: auto;'>
    			<tr>
    				...
    				<td class="editTd">
    					<select class="easyui-combobox" name="crudObject.existedNR">
    				...
    				<td class="editTd">
    					<select class="easyui-combobox" name="crudObject.adjustedOR">
    						<s:iterator value='#@java.util.LinkedHashMap@{"0":"是","1":"否"}' id="entry">
    			...
    			<tr>
    				...
    				<td class="editTd">
    					<s:textfield id="illustration" name="crudObject.illustration" maxlength="1000" cssClass="noborder required"></s:textfield>
    				</td>
    			</tr>
    		</table>
    		</div>
    	...
    	<div id="tt" class="easyui-tabs" fit="true" border="0" style="overflow: hidden;display: none">
    		<s:if test="${crudObject.existedNR == '0'}">
    			<div title="新增风险列表">
					<iframe id="myFrame1"
						src="${contextPath}/riskManagement/management/riskRegister/listRegisterNew.action?riskType=${riskCollectTask.riskType}&collectTaskName=${riskCollectTask.taskName}&collectTaskYear=${riskCollectTask.year}&collectTaskId=${crudObject.formId}"
