		...
		try{
			...
			if(rows && rows.length){
				...
				if(rt){
			    	...
			    	sendData = $.extend({"beanName":"SyBasicsParse"}, insertRowsJson, updateRowsJson);
					$.ajax({
...
/*	var parseZipBtn = {
	...
	if(isView){
		...
		subToolbar = ['search','-',mxAllBtn,'-',saveBtn,'-',addBtn,'-',copyBtn,'-','delete','-','export','-','reload',helpBtn];
	}
	cusToolbar = [];
	...
	function importZipFile(){
		...
		$("#importForm").form('submit',{
			url:'${contextPath}/syBasicsParse/parseSyZip.action',
			onSubmit:function(){
				try{
					aud$loadOpen();	
				}catch(e){}
