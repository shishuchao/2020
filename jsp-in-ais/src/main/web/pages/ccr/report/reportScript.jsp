...
<SCRIPT LANGUAGE="vbscript" FOR="CellWeb1"
			EVENT="SelChanged(oldCol , oldRow, newCol, newRow)">  
    formulaName.value =CellWeb1.GetFormula(newCol, newRow , CellWeb1.GetCurSheet)
    formulaName.title=formulaName.value
</SCRIPT>
...
function setCellPosition(){
	CellWeb1.style.top = idTBDesign.offsetTop + idTBDesign.offsetHeight;
		...
		function submitDate(params,name,rettype,col,row,sheet){
		    ...
		    xmlhttp.open("post", "<%=request.getContextPath()%>/ccReport/loadDataByFormula.action?year=${year}&startMon=${startMon}&endMon=${endMon}&id=${id}", false);
