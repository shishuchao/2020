...
<body onload="createAudAdvice()" class="easyui-layout">
	<s:hidden id="audAdviceFileId" name="generatedDoc.fileId" />
	<s:hidden id="projectName" name="generatedDoc.fileName"/>
	<s:hidden id="projectId" name="projectId"/>
	<div id="buttonDiv" align="center">
		<label for="fileNameInput">文件名</label>
		<input id="fileNameInput" value="${generatedDoc.fileName}" style="width:400px;"/>
        <s:if test="%{mode!='readonly'}">
        	<a class="easyui-linkbutton" data-options="iconCls:'icon-save'" onclick="saveAudAdvice()" >保存文档</a>
        </s:if>
        <%-- <a href="javascript:void(0);" onclick="javascript:WebOffice.style.display='none';showPopWin('${contextPath}/msg/sendMail.action?innerMsg.attachmentsId=${generatedDoc.guid}',500,350,'发送邮件')" >
        [发送邮件]</a> --%>
...
<script type="text/javascript">
	...
	function createAudAdvice(){
		...
		try{
			...
			WebOffice.FileName=projectName;
	...
	function saveAudAdvice(){
        ...
        var fileId = document.getElementById('audAdviceFileId').value;
        ...
        if(fileName) {
            ...
            DWREngine.setAsync(false);DWRActionUtil.execute({
            	...
            	/* var parentURL =document.getElementById('parentURL').value;
            	var superWin = window.dialogArguments;
				superWin.location.href=unescape(parentURL); */
            });
        }
	}
