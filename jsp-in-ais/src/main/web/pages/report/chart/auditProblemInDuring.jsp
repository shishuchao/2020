...
<script language="javascript">
			...
			function CheckSubmit(){
				...
				document.forms[0].action = "auditProblemChartAction!inDuring.action";
				...
				if(problemId!=""){
					...
					function xxx(rs){
						...
						if(null!=data&&data.length>0){
							...
							var problemPoint = document.getElementById("problemPoint");
							var problemUnit = document.getElementById("problemUnit");
							optionsClear(smallType);
							optionsClear(problemPoint);
							...
							for(var i in data){
								if(data[i].isSort=='Y'){
									...
									op.unitName = data[i].unit_name;
									op.unit = data[i].unit;
									problemPoint.options.add(op);
								}
							}
					    }
					}
			...
			function changePoint(){
				var problemPoint = document.getElementById("problemPoint");
				if(problemPoint!=""){
					document.getElementById("problemName").value=problemPoint.options[problemPoint.selectedIndex].text;
					var op = problemPoint.options[problemPoint.selectedIndex];
					var unit = op.unit.split(",");
					var unitName = op.unitName.split(",");
...
<div region="north" title="查询条件" data-options="split:false" style="height:175px;overflow:hidden;">
<s:form name="form">
<table id="tab1" cellpadding=0 cellspacing=0 border=0 align="center" class="ListTable" >
		...
		<tr>
			<td align="left" class="EditHead">比较周期：<FONT color=red>*</FONT></td>
			<td align="left" class="editTd">
				<select name="duringType" id="duringType" class="easyui-combobox" style="width:80%">
