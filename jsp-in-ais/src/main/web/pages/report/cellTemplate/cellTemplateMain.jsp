...
<s:include value="cellTemplateScript.jsp"></s:include>

<script type="text/javascript">
		function CellWeb1_SaveFile(){
			<s:if test="${cellTemplate.templateType==2}">
				var area = GetAreaArray();
				if(area=="false"){
					alert("请设置数据区域开始行变量后保存！");
					return;
				}
			</s:if>
			var result=CellWeb1.UploadFile("http://<%=request.getServerName() + ":" + request.getServerPort()%>${contextPath}/report/ct/saveSign.action?id=${id}");
			if(result==1){
				alert("保存成功！");
			}
		}
		function openSingFile(){
			...
			var result = CellWeb1.OpenFile("http://<%=request.getServerName() + ":" + request.getServerPort()%>${contextPath}/report/ct/readSign.action?id=${id}&param="+rnm,"");
			if(result==1){
...
<s:include value="cellTemplateToolMenu.jsp"></s:include>
