...
<html>
	<head>
		<title>
			<s:if test="${condition.reportType==1}">
				本级报表
			</s:if>
			<s:elseif test="${condition.reportType==2}">
				汇总报表
			</s:elseif>
			<s:elseif test="${condition.reportType==3}">
				调整报表
			</s:elseif>
			<s:elseif test="${condition.reportType==4}">
				下级上报
			</s:elseif>
		</title>
		...
		<center>
			<display:table requestURI="list.action" name="stores" id="row" 
				...
				<display:column title="报表类型" sortName="false" headerClass="center" style="text-align:center">
					...
					<s:elseif test="${row.reportType==4}">
						下级上报
					</s:elseif>
				</display:column>
			...
			<br>
			<div align="right">
				<s:if test="${condition.reportType==1 or condition.reportType==4}">
					<input type="button" value="汇总" onclick="alert(1)">
				</s:if>
				<s:if test="${condition.reportType==1 or condition.reportType==2}">
					<input type="button" value="上报" onclick="alert(1)">
				</s:if>
			</div>
		</center>
	</body>
