...
<html>
	...
	<script type="text/javascript">
		...
		function back(){
			window.location.href="${contextPath}/archives/pigeonhole/searchArchivesList4admin.action";
		}
...
<body class="easyui-layout">
	<div region="center" fit="true">
		<div class="easyui-tabs" fit="true" id="tabs">
			...
			<s:if test="${projectStartObject != null && projectStartObject.xmType != 'syOff' &&  projectStartObject.xmType != 'history'}">
			  <div title="审计成果附件" fit="true" style="overflow: hidden;" onclick="openAttachments();">
				<iframe id="attachments" width="100%" height="100%" frameborder="0" scrolling="auto"></iframe>
			  </div>
			</s:if>
...
<script>
	function readMenuScriptArchives(project_id,contextPath){
		...
		aud$openNewTab('档案查阅-审计底稿',url,false);
	}
	...
	$(function(){
		...
		$('#tabs').tabs({
			onSelect:function(title,index){
			    if(index == 1&&!$('#attachments').data('loaded')){
                    $('#attachments').attr('src','${contextPath}/archives/workprogram/pigeonhole/projectAttachments.action?projectId=${project_id}');
                    $('#attachments').data('loaded',true);
				}
			}
		})
	});
