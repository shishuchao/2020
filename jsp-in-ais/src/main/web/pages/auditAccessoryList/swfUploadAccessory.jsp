...
<html>
	...
	<form name="tempForm" method="post">
		...
		<s:form action="saveAccessory" method="POST" namespace="/auditAccessoryList"
			...
			<div align="center">
				<display:table name="fileList" id="row" class="its" pagesize="10"
					requestURI="/auditAccessoryList/welcomeUpload.action" size="100%">
...
<script type="text/javascript">
		...
		window.onload = function() {						
			var settings = {
				...
				upload_url: "${contextPath}/auditAccessoryList/saveAccessory.action",			//pload_url设置接收的是一个绝对的或者相对于SWF文件的完整URL。推荐使用完整的绝对路径，以避免由浏览器和FlashPlayer修改了基准路径设置而造成的请求路径错误。
				post_params: {"aaid" : "${aaid}","table_name":"${table_name}","templateGuid":"${templateGuid}"},			//定义的是一个包含值对的object类型数据，每个文件上传的时候，其中的值对都会被一同发送到服务端。
				...
				file_size_limit : "100 MB",			//设置文件选择对话框的文件大小过滤规则，该属性可接收一个带单位的数值，可用的单位有B,KB,MB,GB。如果忽略了单位，那么默认使用KB。特殊值0表示文件大小无限制。
				file_types : "*.*",			//设置文件选择对话框的文件类型过滤规则，该属性接收的是以分号分隔的文件类型扩展名，例如“ *.jpg;*.gif”，则只允许用户在文件选择对话框中可见并可选jpg和gif类型的文件。默认接收所有类型的文件。
				...
				button_text_style: ".theFont { font-size: 12; }",			//默认值："color: #000000; font-size: 16pt;"(v2.2.0新增)此参数配合button_text参数，可以通过CSS样式来设置Flash Button中的文字样式。
	     ...
	     function reloadParentFileList(){
	     	if(window.parent.parent.document.getElementById("${idName}")!=null){
					window.parent.parent.document.getElementById("${idName}").innerHTML='${accessoryList}';
			}
	     }
