...
function Load(){
  ...
  <%
		List bmList = (List)request.getAttribute("templateBookMarkList");
  		...
  		if(manuIdArr.length==0){
  			%>
  			top.$.messager.show({
					title:"提示信息",
					msg:"没有选中底稿，请在上一页面进行选择！",
					timeout:5000,
				});
  			self.close();
  			<%
...
function WebOpenLocal(){
  try{
	...
	<body bgcolor="#ffffff" onload="Load()" onunload="UnLoad()" style='overflow:hidden;padding:0px;margin:0px;'>
		<!--引导和退出iWebOffice-->
			...
			<table cellpadding=0 cellspacing=0 border=0 bordercolor="red"
				...
				<tr>
					<td align=right class="listtabletr11" height="30">
						<div style='float:left;line-height:30px;height:30px;padding:1px 2px 1px 5px;'>导出单项底稿：<label  id=StatusBar></label></div>
						<div style='float:right;padding:2px 5px 1px 2px;'>
							...
							<a id="btn" href="javascript:;" onclick="WebSaveLocal()" class="easyui-linkbutton" data-options="iconCls:'icon-add'">保存本地文件</a>
							<a id="btn" href="javascript:;" onclick="WebOpenPrint()" class="easyui-linkbutton" data-options="iconCls:'icon-search'">打印文件</a>
							<a id="btn" href="javascript:;" onclick="closeMulti()" class="easyui-linkbutton" data-options="iconCls:'icon-cancel'">关闭</a>
						</div>
					</td>
				</tr>
				<tr>
