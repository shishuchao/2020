...
//传过来的记录主键，多个，以逗号分隔
	String evidenceIds = (String) request.getAttribute("crudId");
	String[] evidenceIdArr = {};
	if (!"".equals(evidenceIds) && evidenceIds != null) {
		evidenceIdArr = evidenceIds.split(",");
	}
%>
<html>
	<head>
	...
	<body onload="createEvidence()">
		<s:form id="webform" name="webform" action="expEvidence" namespace="/project/evidence">
		...
		*/
		function createEvidence(){
			...
			try{
				...
				webform.WebOffice.FileName=projectName+"取证记录.doc";
				...
				//测试复制模板
				  ...
				  //如果选择了多个底稿，则对模板进行复制操作
				  	if(evidenceIdArr.length>1){%>
				  		...
				  		countFormula('<%=evidenceIdArr[0]%>');
				  	<%
				  		for(int i=1;i<evidenceIdArr.length;i++){
				  			String evidenceId=evidenceIdArr[i];
				    %>
							  try{
