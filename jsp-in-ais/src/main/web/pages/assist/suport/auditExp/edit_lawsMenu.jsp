	...
	function mydelSubmit(){
		if(true){  //mydelCheck()
			if(confirm("确认删除吗？")){
				...
				//lawsMenuForm.submit();
				window.location.href="../lawsLib/delMenu.action?assistSuportLawslibMenu.id=${assistSuportLawslibMenu.id}&assistSuportLawslibMenu.classification=${assistSuportLawslibMenu.classification}";
			}else{
				return false;
			}
		...
		<s:form action="saveMenu" namespace="/pages/assist/suport/lawsLib" name="lawsMenuForm"
			...
			<table 
				...
				<tr>
					...
					<td class="listtabletr22">
					<%--<s:if test="${empty(m_view)}">
						<s:textfield name="assistSuportLawslibMenu.create_man"
							value="%{assistSuportLawslibMenu.create_man}" readonly="true"></s:textfield>
						</s:if>
						--%>
				...
				<tr>
					...
					<td class="listtabletr22">
					<%--<s:if test="${empty(m_view)}">
						<s:textfield name="assistSuportLawslibMenu.create_date" value="${fn:substring(assistSuportLawslibMenu.create_date, 0, 10)}" readonly="true"></s:textfield>
					</s:if>
							--%>
					...
					<tr>
						<td>
							<span style="float:right;">
								...
								<s:button value="重置" onclick="return clearInfo();"/>
							</span>
						</td>
					</tr>
				</table>
