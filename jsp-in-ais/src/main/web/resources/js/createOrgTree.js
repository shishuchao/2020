 ...
 * 根据后台的hibernate bean信息来配置树形、表格，后台代码基本不用写，页面简单配置即可。
    ...
    ************************************************************************************
    ...
    @Date   :2017.08.17
    @Author :qfucee 
    1.Qdatagrid添加单独的删除方法，删除方法添加noMsg参数，noMsg:true则没有confirm
     添加deleteAll方法,指定删除几行，从末尾开始，不指定就删除全部
     createQDatagrid.prototype.deleteAll = function(noMsg, count)
     
    2017.08.31
    1.解决datagrid分页时，连续请求两次url
    
    2017.09.01
    1.解决Qdatagrid的refresh，刷新的时候每次都是跳到第一页，使用自定义的ajax方法代码load方法
    
   	@Version:3.2.5
    @Date   :2017.09.15
    @Author :qfucee 
    1.解决Qdatagrid的refresh:pagination("options").total != data.total 时，会触发refresh，导致多次加载。
    pageOpt.total记录时上次的datagrid数据的条数。例如初始化时datagrid有5条数据，通过某个查询条件后，没有查询到数据，
    (refresh调用loadData)通过$(gridObject).datagrid('loadData',data)加载数据时，如果data.rows.length == 0
    会判断和pagination("options").total是否相等，如果不相等会重新加载数据（加载三次）               
    page.pagination("refresh", {
        total: data.total
    });
    解决办法是：Qdatagrid的refresh加载成功后，success方法中如果data.rows.length == 0,把pagination("options").total = 0手动
    设置为0
    
    @Version:3.2.7
    @Date   :2017.11.1
    @Author :qfucee 
    1.扩展Array，增加unique，removeByValue。
	2.createQDatagrid中统一pageSize， 外层如果有pageSize就按外层的，如果gridJson中有pageSize就按内层，
	内层的pageSize优先级更高。（查询，refresh都按此分页）2017.10.30
	3.qdatagrid的查询表单input的width默认85%。2017.10.31
	4.qdatagrid.refresh = function(cusParam, isResetQuery2Default),添加参数isResetQuery2Default，
	  刷新时是否清除上次的查询条件，否则只在上次查询的结果内查询或者刷新
	  添加新方法：resetQuery2Default，回复查询条件到初始状态; 
	  添加新方法：QUtil.showJson(paramJson); 2017.11.1
	  
    @Version:3.2.8
    @Date   :2017.11.7
    @Author :qfucee 
	1.createQDatagrid中 columns -> field中添加merge，用于合并指定的列
	    columns:[[{field:'cityName', title:'所在城市',  merge:true},...]]
		添加getMergeCols,merge方法，默认是放到onloadSuccess中的，就可以合并columns中有merge：true的field，如果自定义了
		onloadSuccess，就需要自己调用merge方法
		var g1 = new createQDatagrid({
			...
			gridJson:{
				...
				onLoadSuccess:function(data){
					//合并指定列的相同项，data可以不填，如果有加上就省的重新获取
					g1.merge(data);
				},
				...
			}
		});
		.添加全局属性exportNoMerge：true/false导出到excel时，是否合并列
	2.CommonExcelUtil导出excel方法添加合并列方法	
	
	@Version:3.2.9
    @Date   :2017.11.8
    @Author :qfucee 
	1.createQDatagrid中添加全局属性selectColum，类型为数组，selectColum的值为所有columns:[[{field:...值(包括冻结)和自己指定的field
	  后台查询时，会根据selectColum是否有值来指定查询字段，如果selectColum为空，则查询全部;
	  添加全局属性 selectAll，类型为boolean，true/false, true:select * from 查询全部字段；false：仅查询grid用到的字段和selectColum指定的字段；
	  columns:[[{field:...添加real属性，布尔值true/false，默认值为true，如果属性字段是虚拟字段，real要设置为false;
	  
	@Version:3.3.0
    @Date   :2017.12.11
    @Author :qfucee 
    1.纠正 createQDatagrid 全局属性selectColum不起作用问题
	
	@Version:3.3.1
    @Date   :2017.12.13
    @Author :qfucee 
	1.createQDatagrid添加全局查询参数变量
	var gqueryParams = paramJson.gqueryParams ? paramJson.gqueryParams : {
		'query_eq_planType':'1',
		'customParam':"exists (select 1 from xxx where ...'
	};
	和whereSql组成最基础的查询条件（如果查询form中的name值和gqueryParams的值相同，会覆盖gqueryParams，此时如果不想被覆盖，可以把
	此条件写在whereSql中）
	2.createQDatagrid添加自定义的close按钮
    ...
    @Date   :2017.12.15
    @Author :qfucee 
	1.createQDatagrid的refresh添加callbackFn回调函数，用于刷新页面列表后执行这个，如果关闭窗口。由于这个ajax异步，如果没有则可造成
	刷新没有完成时，关闭了窗口，造成错误。
    ...
    @Author :qfucee	
	1.datagrid增加列拖动功能。
	  增加属性 columnMoving:true/false启用或者禁用列拖动，默认false
	  
  	@Version:3.3.4
    @Date   :2018.5.25
    @Author :qfucee	
	1.Qdatagrid的columns列属性字段，增加用户自定义的导出exportTitle和exportField，如：
	{field:'implStatusCode',title:"实施状态", exportField:'implStatusName',..}, 默认显示的code，我们希望导出时是name
	如果没有这个两个字段，默认取title和field
	2.Qdatagrid查询form中根据form表单元素是否有noClear=true来判断，点击清空按钮后是否清除此元素的值。
	
	@Version:3.3.5
    @Date   :2018.7.02
    @Author :qfucee	
    1.Qdatagrid查询并导出，应该导出查询的结果集，而是初始化时的结果集。
	
	@Version:3.3.6
	@Date   :2018.7.19
	@Author :qfucee
	1.Qdatagrid添加参数onceParams：只在初始化第一次时使用的参数，不参加字段排序，分页和查询
	适合表格第一次初始化的条件，而表单查询时又要覆盖此条件的情况，如：
	某表单需要查询2018年的记录，而查询表单查询条件可以查询2010-2020年的所有记录，这种情况就可以把年度这个查询条件放到onceParams中，
	以前的查询如果表格初始查询条件限定为2018，那么表单查询条件无论选择什么，那只能查询2018的。
    
	@Version:3.3.7
	@Date   :2018.8.29
	@Author :qfucee
	1.树形窗口取消collapsible、resizeable；已选择tab页签，名称列宽度设置为79%，增加title提示；
	2.解决cascadePrior、cascadeJunior时，级联选择、单选、全部选中、全部取消不起作用问题。
	
	2018.9.4
	1.Qdatagrid:refresh添加等待进度提示；
	  export2Excel导出到excel添加进度提示；
	2018.9.11
	1.jqtree加载时，确定时，对nodeId进行左右去空格操作；
	2018.9.20
	1.Qdatagrid:删除时添加等待进度条提示；  
	2018.9.29
	1.Qdatagrid:解决无法分页时，刷新按钮失效；
	
	@Version:3.3.8
	@Date   :2018.11.09
	@Author :qfucee
	1.优化Qdatagrid查询表单属性，根据columns的以下属性生成查询表单：
	reqired： true/false, 是否必填；
	validType：查询表单校验类型；如：len,number,money，具体类型查看tlInteractiveCommon.js文件；
	type：date 生成日期控件查询；
	type：dateBt/datebt 生成日期前后范围控件（两个日期控件），并要求后面的日期要大于等于前面的日期；
	2.查询表单提交通过tlInteractiveCommon.js的audEvd$validateform方法校验表单（如果此文件已经引用的话）
	
	@Version:3.3.8.1，2018.11.13，qfucee
	1.Qdatagrid：column添加属性singleRow:true/false, 用于生成的查询form的查询字段是否单行显示；
 */
//------------------------------------- 基本类的扩充 ------------------------------------------
...
function createOrgTree(optionJson){
    ...
    // 初始化参数
    this.option = {
    	cuscascade:optionJson.cuscascade,
        ...
        'employeeColumns':[  
            ...
            {field:'name',          title:'姓名',       width:'80px', halign:'center',align:'center', sortable:false},
            {field:'sysAccounts',   title:'系统账号',   width:'80px',  halign:'center',align:'center', sortable:false},                    
            {field:'company',       title:'所在单位',   width:'40%', halign:'center',align:'center', sortable:false,
        ...
        'employeeSelectedColumns':[  
            ...
            {field:'company',        title:'所在单位', width:'180px', halign:'center',align:'left'}
        ],
        'economyDutyColumns':[  
            ...
            {field:'company',       title:'所在单位',   width:'230px', halign:'center',align:'left',  sortable:false}
        ],
        'economyDutySelectedColumns':[  
            {field:'id'  ,          title:'选择',       width:'10px',  checkbox:true, halign:'center', align:'center'},
            {field:'name',          title:'姓名',       width:'240px',  halign:'center', align:'left'},
            {field:'company',       title:'所在单位',   width:'300px',  halign:'center', align:'left',  sortable:false,hidden:true}
        ],
		'treeNodeSelectedColumns':[  
            {field:'id'  ,          title:'选择',       width:'10px',  checkbox:true, halign:'center', align:'center'},
            {field:'name',          title:'名称',       width:'70%',  halign:'center', align:'left', 
			formatter:function(value,row,index){
				return  ["<label title='单击【"+value+"】导航到树形节点' style='cursor:pointer;color:blue;'>",value,"</label>"].join('') ;
			}}
        ]
    };
    this.storeData = null;
...
createOrgTree.prototype.createWin = function(){
    try{
        ...
        if(!isWinPlug){
            ...
            //alert(containerW);
            queryInputWidth = containerW && (containerW - queryBtnWOffset)<=135 ? containerW - queryBtnWOffset : 110;
        ...
        if(othis.option.showTabTools){
            ...
            if(othis.option.type === 'tree' && !isWinPlug && othis.option.treeQueryBox){
                $("<div data-options=\"iconCls:'icon-search'\" style='padding: 0px;'>节点搜索</div> ").appendTo(menuList).bind('click', function(){
                    $(footBtnWrap).show();
                });  
                $("<div class='menu-sep'></div>").appendTo(menuList); 
            }
            $("<div data-options=\"iconCls:'icon-expand'\" style='padding: 0px;'>展开全部</div> ").appendTo(menuList).bind('click', function(){
                QUtil.expandOrCollapseAllNodes(jqtree,$(jqtree).tree('getRoot'));
            });   
            $("<div data-options=\"iconCls:'icon-arrow-down'\" style='padding: 0px;'>展开选中</div> ").appendTo(menuList).bind('click', function(){
                QUtil.expandOrCollapseAllNodes(jqtree);
            });       
            $("<div class='menu-sep'></div>").appendTo(menuList);
            $("<div data-options=\"iconCls:'icon-collapse2'\" style='padding: 0px;'>折叠全部</div> ").appendTo(menuList).bind('click', function(){
                QUtil.expandOrCollapseAllNodes(jqtree,$(jqtree).tree('getRoot'),'collapse');
            });   
            $("<div data-options=\"iconCls:'icon-arrow-up'\" style='padding: 0px;'>折叠选中</div> ").appendTo(menuList).bind('click', function(){
                QUtil.expandOrCollapseAllNodes(jqtree, $(jqtree).tree('getSelected'), 'collapse');
            });    
            $("<div class='menu-sep'></div>").appendTo(menuList);
            $("<div data-options=\"iconCls:'icon-reload'\" style='padding: 0px;'>重新加载</div> ").appendTo(menuList).bind('click', function(){
                $(jqtree).tree('reload');
            }); 
			...
			*/
            ...
            $("<div data-options=\"iconCls:'icon-clean'\" style='padding: 0px;'>清空缓存</div> ").appendTo(menuList).bind('click', function(){
            ...
            if(othis.option.treeCheckbox){           		
        		var cuscascade = false;
        		try{        			
        			cuscascade = othis.option.cuscascade;
        			//alert("cuscascade="+cuscascade)
        			cuscascade = cuscascade == true ? true : false;
        		}catch(e){}
        		
        		$("<div class='menu-sep'></div>").appendTo(menuList); 
        		$("<div data-options=\"iconCls:'icon-cascadeOpen'\" style='padding: 0px;font-weight:"+(!cuscascade ? "normal" : "bold")+";'>级联选择</div> ").appendTo(menuList).bind('click', function(){
        			$(jqtree).tree('options').cuscascade = true;
        			$(jqtree).tree('options').cascadeJunior = true;
        			$(this).find('div:first').css("font-weight","bold");
        			$(this).next().find('div:first').css("font-weight","normal");
        		});   
        		$("<div data-options=\"iconCls:'icon-cascadeCancel'\" style='padding: 0px;font-weight:"+(cuscascade ? "normal" : "bold")+";'>单级选择</div> ").appendTo(menuList).bind('click', function(){
        		...
        		$("<div class='menu-sep'></div>").appendTo(menuList); 
        		$("<div data-options=\"iconCls:'icon-checkbox1'\" style='padding: 0px;'>全部选中</div> ").appendTo(menuList).bind('click', function(){
        			checkAllOrCancel(jqtree, true);
        		}); 
        		$("<div data-options=\"iconCls:'icon-checkbox0'\" style='padding: 0px;'>取消选中</div> ").appendTo(menuList).bind('click', function(){
        			checkAllOrCancel(jqtree, false);
        		}); 
                
                function checkAllOrCancel(jqtree, checked){
                	var ischange = false;
                	//alert("cuscascade="+$(jqtree).tree('options').cuscascade+"\ncascadeJunior="+$(jqtree).tree('options').cascadeJunior)
                	if(!$(jqtree).tree('options').cuscascade){  
                		ischange = true;
                		$(jqtree).tree('options').cuscascade = true;
                		$(jqtree).tree('options').cascadeJunior = true;
                	}
...
createOrgTree.prototype.getTreeAndEmployeeLayout = function(winWrap,headWrap,queryWrap,btnWrap,treeWrap){
    try{
        ...
        // 缓存查询参数
        $(othis.tableInfoWrap).data('queryParamsJson',queryParamsJson).data("selectedGrid",selected_tableinfoWrap);
...
var QUtil = {
                ...
                */
                ...
                if(!idArr || idArr.length == 0){
					...
					if(!idArr || idArr.length == 0){
						...
						var node = $(jqtreeObject).tree('find',targetNodeId);
						if(node) {
							idArr[0] = node.id;
							var i = 1;
							while ($(jqtreeObject).tree('getParent', node.target) != undefined && $(jqtreeObject).tree('getParent', node.target) != null) {
								var parent = $(jqtreeObject).tree('getParent', node.target);
								node = parent;
								idArr[i++] = parent.id;
							}
						}
    ...
    treeNodeLoadSuccess:function(node,data,cache,treeOption,winOption,treeParam,idsInput){
		...
		if(checkedNodes && checkedNodes.length){
			for (var i = 0;i < checkedNodes.length;i++){
				if(jQuery.inArray(checkedNodes[i].id, ids) == -1) {
		...
		if(ids && ids.length){
			...
			if(action === 'check'){
				$.each(children,function(i, node){
					if(jQuery.inArray(node.id, ids) == -1){
            ...
            $(winOption.addbtn).bind('click',function(){
                ...
                if(type === 'tree'){
                    ...
                    $.each(nodes, function(i,node){
                        ...
                        dms.push($.trim(node.id));
                        mcs.push($.trim(node.text));
                    }); 
                }else{// 既有机构又有选人的情况
                        ...
                        if(type && type == 'cache'){// 从已经加载的树形节点搜索
                            ...
                            if(rtArr && rtArr.length > 0){// 缓存查询   
								...
								$.each(rtArr, function(n, rt){
									...
									QUtil.selectedSpecifiedNode(winOption.jqtree, tnode, queryKey, false);
								});
            ...
            function enterHandler(e){
                ...
                if(oEvent){// for IE
                    ...
                    if(oEvent.keyCode == "13"){
                        ...
                        oActiveObjId.indexOf('tableQuery') !=-1 ? ($(winOption.tableQuerybtn).is(':hidden') ? $(winOption.tableQuerybtnDb).trigger('click'):$(winOption.tableQuerybtn).trigger('click')) : null;
                    }
                }
            }
    ...
    loadGrid:function(paramJson){
        try{
            ...
            if($.ajax){
                ...
                var pageOption = null;
                try{                	
                	pageOption = $(gridObject).datagrid('getPager').pagination('options');
                }catch(e){}
                ...
                $.ajax({
                    ...
                    beforeSend:function(){
                    	...
                    	return beforeSendFn ? beforeSendFn() : true;
                    },
                    success: function(data){
                    	try{
                    		aud$loadClose();
                        ...
                        if(data.rows && data.rows.length > 0){
                        	...
                        	//datagrid:.pagination("options").total != data.total 时，会触发refresh，导致多次加载。pageOpt.total记录时上次的datagrid数据的条数
            ...
            if(gridcolumns){
                ...
                if(columns){
                    for(var i=0; i<columns.length; i++){
                        ...
                        // 是否必填
                        var isNotNull = false;
						if(c1.request || c1.required){
							isNotNull = c1.request == true || c1.request == "true" ? true : false; 
							isNotNull = c1.required == true || c1.required == "true" ? true : isNotNull; 
						}
						var queryKey = c1.queryKey ? c1.queryKey : c1.field;
						// 校验类型
						var validType = c1.validType ? c1.validType : "";
                        formAttr.push({
							'queryKey':queryKey,
                            ...
                            'singleRow':c1.singleRow == true || c1.singleRow == 'true' ? true : false
                        });
                    }
                }
                return formAttr;
				...
				if(paramJson.removeFilter){
					...
					$.each(rows, function(i, row){
						if(paramJson.delRefresh != false) {
							...
							gridObj.datagrid('deleteRow', gridObj.datagrid('getRowIndex', row));
						}
    ...
    */
	...
	// datagrid合并指定列的相同项
	mergeDatagridCols:function(gridObject, fieldsName, gridData){
		try{
			if(gridObject && fieldsName && fieldsName.length){
				var data = gridData ? gridData : $(gridObject).datagrid('getData');
					...
					$.each(fieldsName, function(i, field){
						...
						$.each(rows, function(j, row){
							...
							if((val != preVal || j == len - 1) && samecount){ 
								$(gridObject).datagrid('mergeCells',{
									index  : beginIndex,
									field  : field,
									rowspan: samecount,
									colspan: 1
			...
			isdebug ? alert('mergeDatagridCols:\n'+e.message) : null;
		}
	},	
	showJson:function(json){
		try{
			...
			top.$.messager ? top.$.messager.alert("参数信息", s2.join("</br>"),'') : alert(s2.join("\n"));
		}catch(e){
			isdebug ? alert('showJson:\n'+e.message) : null;
		}
	}
};
...
function createQDatagrid(paramJson){
	...
	//全局查询参数
	var gqueryParams = paramJson.gqueryParams ? paramJson.gqueryParams : {};
	// 只在初始化第一次时使用的参数，不参加字段排序，分页和查询
	var onceParams = paramJson.onceParams ? paramJson.onceParams : {};
    var basicGridParam = {
		//是否关联查询
	...
	for(var p in gqueryParams){
		basicGridParam[p] = gqueryParams[p];
	}
	...
	//是否拖动列
	var columnMoving = paramJson.columnMoving == true || paramJson.columnMoving == "true" ? true : false;
	$(gridObject).data('columnMoving',columnMoving);
	
	//是否查询全部字段
	var selectAll = paramJson.selectAll;
	paramJson.selectAll = selectAll == true || selectAll == "true" || selectAll == null || selectAll == undefined ? true : false;
	
	//获得要合并的列
	this.getMergeCols();
	if(!paramJson.selectAll){
		var selectColum = $(gridObject).data('selectColum');
		selectColum = selectColum && selectColum.length ? selectColum : [];
		var  basicSelectColum = paramJson.selectColum;
		basicSelectColum = basicSelectColum && basicSelectColum.length ? basicSelectColum : [];
		basicSelectColum.push.apply(basicSelectColum, selectColum);
		if(basicSelectColum && basicSelectColum.length){
			basicGridParam['selectColumStr'] = basicSelectColum.join(",");
		}
	}
    // 单独复制一个查询json，防止grid初始化的参数覆盖basicGridParam
    var defaultGridQueryParams = jQuery.extend({},basicGridParam, onceParams);
    //QUtil.showJson(defaultGridQueryParams)
    ...
    var pz = paramJson.gridJson.pageSize ? paramJson.gridJson.pageSize : basicGridParam.rows;
	tmpJson.pageSize = pz;
	this.defaultPageSize = pz;
	basicGridParam.rows = pz;
	...
	var barbtnJson = {
		'delete':{
            ...
            id     :tdomId+'_remove',
            handler:function(){
                return QUtil.removeDatagridRows(paramJson, paramJson.noDelMsg);
            }
        }, 
		'export':{
            text:'导出到Excel',
	...
	*/
    ...
    setTimeout(function(){
    	$(gridObject).datagrid('resize', {});
    },200);
}

//获得要合并的列
createQDatagrid.prototype.getMergeCols = function(){
	var merges = [];
	var fields = [];
	try{
		var gridObject = this.gridObject;
		var paramJson = $(gridObject).data('paramJson');
		if(paramJson && paramJson.gridJson){
			var gridJson = paramJson.gridJson;
			var frozenTs = gridJson.frozenColumns;
			...
			$.each(qCols, function(i, col){
				var m = col['merge'];
				if(m == 'true' || m == true ){
					merges.push(col['field']);
				}
				var r = col['real'];
				if(r != false && r != 'false'){
					//alert(col['field']+" "+r)
		...
		$(gridObject).data('selectColum', fields);
		$(gridObject).data('merges', merges);
		return merges;
    }catch(e){
        isdebug ? alert('getMergeCols:\n'+e.message) : null;
    }
}

//合并列
createQDatagrid.prototype.merge = function(data){
	try{
		var gridObject = this.gridObject;
		var merges = $(gridObject).data('merges');
		if(merges && merges.length){
			QUtil.mergeDatagridCols(gridObject, merges, data && data.length ? data : null);
		}
    }catch(e){
        isdebug ? alert('merge:\n'+e.message) : null;
    }
...
//重置查询条件到初始状态
createQDatagrid.prototype.resetQuery2Default = function(){
    try{
		var gridObject = this.gridObject;
		var basicGridParam = $(gridObject).data('basicGridParam');
		...
		var qoptions = othis.isTreeGrid ? $(gridObject).treegrid('options') : $(gridObject).datagrid('options');
		qoptions.queryParams = tmpJson;
    }catch(e){
        isdebug ? alert('resetQuery2Default:\n'+e.message) : null;
    }
}
...
createQDatagrid.prototype.export2Excel = function(paramJson,typeAction){
    try{
        ...
        for(var i=0; i<arr.length; i++){
            ...
            //选择列不导出 by zxl 20.3.2
            if((obj.title==null || obj.title=='') && obj.checkbox) continue;
            if(!obj.hidden){
                names.push(obj.exportTitle ? obj.exportTitle : obj.title);
                colums.push(obj.exportField ? obj.exportField : obj.field);
                dataTypes.push(obj.exportType ? obj.exportType : ""); //增加导出字段的类型，用来格式化EXCEL单元格 by zxl 19.4.28
            }
        }
        // excel表格的抬头和对应的数据库表字段,以及要导出的字段的类型
        var data = {
            'colNames':names.join(','),
            'colatrrs':colums.join(','),
			'colDataTypes':dataTypes.join(',')
        };
		
		var merges = $(gridObject).data('merges');
		if(merges && merges.length && !paramJson.exportNoMerge){
			data['merges'] = merges.join(",");
		}
    ...
    try{
        ...
        if(gridObject){
			if(this.isTreeGrid){
				...
				if($(gridObject).data("columnMoving") == true){
					$(gridObject).datagrid("columnMoving");
				}
			}
            //设置分页控件  
            ...
            $(p).pagination({
                onChangePageSize:function(pageSize){
					...
					//QUtil.showJson(gridQueryParamJson);
                },
                showRefresh:true,
                showPageList:true,
            ...
            //$(gridObject).datagrid('getPanel').removeClass('lines-both lines-no lines-right lines-bottom').addClass("lines-bottom");
        }
    }catch(e){
...
createQDatagrid.prototype.createSerachWindow = function(gridObject, wincloseFn, winopenFn, winW, winH, tdomId, fnJson){
    try{
        ...
        if(!this.winDiv){
           ...
           $(searchForm).attr('id','qdatagridSearchForm'+QUtil.ran(6))
           function queryForm(actionType){
				...
				var searchFormId = $(othis.searchForm).attr('id');
				//alert(searchFormId+"\n"+othis.searchForm.id+"\n"+audEvd$validateform)
                if(winWhenClickSearchBtnFn && jQuery.type(winWhenClickSearchBtnFn) == "function"){
                    continueSearch = winWhenClickSearchBtnFn();
                }else if(audEvd$validateform && searchFormId){
					//alert('valid')
					continueSearch = audEvd$validateform(searchFormId);
				}
                ...
                if(continueSearch){
                    ...
                    if(requestInputObjArr && requestInputObjArr.length>0){
                        for(var i=0; i<requestInputObjArr.length; i++){
                            ...
                            if(!$(obj).val()){
								if(top.$ && top.$.messager){
									top.$.messager.alert('提示',"["+$(obj).attr('title')+"]为必填项，不能为空",'warning');
								}else{
					...
					//QUtil.showJson(paramJson);
					var qoptions = othis.isTreeGrid ? $(gridObject).treegrid('options') : $(gridObject).datagrid('options');
					qoptions.queryParams = gridQueryParamJson;
           ...
           $(clearBtn).html('清空').linkbutton({
				...
				$(othis.searchForm).find('input:not([noClear]), select:not([noClear]), textarea:not([noClear])').val('');
...
createQDatagrid.prototype.createSerachForm = function(paramJson, tdomId){
    try{
        ...
        if(gridObject){     
            ...
            $(this.searchForm).append(table).css({
            	"overflow":"hidden",
				"padding":"0px",
				"margin" :"0px",
				"border" :"0px"
            ...
            if(formArr && formArr.length > 0){
                var columCount = formArr.length;
            	var curTr = null;
            	var colspanSum = 0;
            	var queryColspan = parseInt(colcount) + 1;
                $.each(formArr,function(i, json){
                	i = parseInt(i);
                	var singleRow = json.singleRow;
                    //alert(i%2+'\n'+json.type+'\n'+json.text+'='+json.name);
                    //alert(othis.searchForm.innerHTML)
                    var type = json.type;
                    //alert("i="+i+"\ncolspanSum="+colspanSum)
                    if((i + colspanSum) % colcount == 0){
                        curTr = document.createElement('tr');
                        $(table).append(curTr);
                        if(singleRow){                        	
                        	colspanSum = parseInt(colspanSum) + queryColspan;
                        }
                    }else{
                    	singleRow = false;
                    }
                    if(json.request){
                        $(curTr).append("<td class='EditHead' nowrap style='width:16%;border-left-width:0px;'><font color='red'>*</font>&nbsp;"+json.text+"</td>");
                    }else{
                        $(curTr).append("<td class='EditHead' nowrap style='width:16%;border-left-width:0px;'>"+json.text+"</td>");
                    }
                    var td = document.createElement('td');
                    $(curTr).append(td);
					if(i < colcount){
						$(curTr).find('td').css('border-top-width','0px');
					}
					
                    var inputObj = document.createElement('input');
                    $(td).attr('class','editTd');
                    // 如果列数colcount ！= 0那么最后一行添加colspan属性
                    if(singleRow || (i == columCount - 1 && $(curTr).find('.EditHead').length < colcount)){
                    	$(td).attr('colspan', queryColspan);
                    }
                    if(type){
                        ...
                        if(type == 'date'){
							...
							if(json.request == true || json.request == 'true'){
                        ...
                        }else if(type == 'dateBt' || type == 'datebt'){
							inputObj = null;
							var dateB1Id = "queryMtq" + json.queryKey;
							var dateB2Id = "queryLtq" + json.queryKey;
							var dateWrap1 = document.createElement('span');
							var dateB1 = document.createElement('input');
							$(dateWrap1).addClass('editElement').append(dateB1);
							$(dateB1).addClass('noborder').attr({
								'id':dateB1Id,
								'name':"query_mtq_" + json.queryKey,
								'title':json.text + '-开始',
								'request':json.request
							}).css('width','100px');
							
							var dateWrap2 = document.createElement('span');
							var dateB2 = document.createElement('input');
							$(dateWrap2).addClass('editElement').append(dateB2);
							$(dateB2).addClass('noborder').attr({
								'id':dateB2Id,
								'name':"query_ltq_" + json.queryKey,
								'title':json.text + '-结束',
								'request':json.request,
								'compareval':'gte&'+dateB1Id
							}).css('width','100px');
							
							if(json.request == true || json.request == 'true'){
								$(dateB1).addClass('required');
								$(dateB2).addClass('required');
							}
							
							$(td).append(dateWrap1).append("&nbsp;至&nbsp;").append(dateWrap2);
...
// 根据查询条件，刷新表格, isResetQuery2Default:true 清除上次的查询条件，恢复到初始的查询条件
createQDatagrid.prototype.refresh = function(cusParam, isResetQuery2Default){
    try{
		var othis = this;		
        var gridObject = this.gridObject;
		...
		//alert(sortJson.sort+" "+sortJson.order)
        basicGridParam =basicGridParam ? basicGridParam: {};
		...
		var loadGridParam = {};
		var baseParam = {};
		if(isResetQuery2Default){//清除上次的非基础查询条件
			othis.resetQuery2Default();
			baseParam = basicGridParam;
		}else{//继续使用上次的查询条件
			//上次的查询条件
			baseParam = $(gridObject).data('gridQueryParamJson');
			baseParam = baseParam ? baseParam : {};
		}
		loadGridParam = jQuery.extend({},baseParam, sortJson, cusParam);
		//缓存查询条件
		$(gridObject).data('gridQueryParamJson', loadGridParam);
		var qoptions = othis.isTreeGrid ? $(gridObject).treegrid('options') : $(gridObject).datagrid('options');
		qoptions.queryParams = loadGridParam;
		//QUtil.showJson(loadGridParam);
		
		var mycallbackFn = cusParam.callbackFn ? cusParam.callbackFn : function(){};
...
//指定删除几行，从末尾开始，不指定就删除全部
createQDatagrid.prototype.deleteAll = function(noMsg, count){
	try{		
		var gridObject = this.gridObject;
		$(gridObject).datagrid('uncheckAll');				
		if(count){
			var rows = $(gridObject).datagrid('getRows');
			if(rows && rows.length){
				var len = rows.length;
				if(count >= len){
					$(gridObject).datagrid('checkAll');
				}else{					
					for(var i = len - 1; i >= len - count; i--){
						$(gridObject).datagrid('checkRow', i);
					}
				}
			}
		}else{
		...
		var paramJson = $(gridObject).data('paramJson');
		QUtil.removeDatagridRows(paramJson, noMsg);
	}catch(e){
		alert(e.message);
	}
}
    ...
    if(targetElement){
		...
		*/
        ...
        var winJson = QUtil.getWin({ 
            ...
            'cuscascade': treeOption.cascadePrior || treeOption.cascadeJunior ? true : false,
            ...
            if(!treeInit  || !cache){// true: 说明控件已经加载过了
               ...
               if(treeParam){
                ...
                $(winOption.jqtree).tree({
                    ...
                    // 选择本上级
                    cascadePrior:treeOption.cascadePrior,
                    // 选择本下级
                    cascadeJunior:treeOption.cascadeJunior,
