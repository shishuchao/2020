	...
	*/
    ...
    function createUploadWin(target, options, isOpenDialog){
        try{
            ...
            var dgH = winsize.height > 600 ? 420 : 420;
        ...
        try {
            ...
            targetWin.$(fileDatagrid).attr('id', "fileUploadViewListTable_"+genRandNum()).datagrid({
                ...
                columns:[[   
                    ...
                    {field:'fileStatus',title:'状态',  width:'60px', halign:'center', align:'center'},
                    {field:'fileSize',  title:'大小',  width:'75px', halign:'center', align:'right' },
                    {field:'fileType',  title:'类型',  width:'60px', halign:'center', align:'center'},
                    {field:'operation', title:'操作',  width:'45px', halign:'center', align:'center', 
                        formatter:function(value, rowData, rowIndex){
    ...
    function setBottom(options, bottomObj){
        try{
            ...
            var str = [];
            str.push("<div style='text-align:right; padding:0px 15px 0px 5px;line-height:30px;background-color:#fafafa;'><input type='text' value='' style='width:0px;border-width:0px;' />");
            if(excludeSuffixs){
                str.push("禁止后缀:");
            ...
            if(maxFileCount){
                str.push("&nbsp;&nbsp;&nbsp;单次最大上传[<font color='red'>");
        ...
        try{
			...
			targetWin.$(options.submitFileForm).form('submit', {
				...
				onSubmit: function(param){
					...
					!rt ? targetWin.$.messager.progress('close') : null;	// 如果表单是无效的则隐藏进度条
					rtflag = rt;
				...
				success: function(data){
					targetWin.$.messager.progress('close');// 如果提交成功则隐藏进度条
					if(data){
	...
	*/
    ...
    function loadFileEchoRows(options, queryParams){
        try{
            ...
            createFileEchoRows(options, fileNameArr, fileIdArr, true);
        }catch(e){
    ...
    function createFileEchoRows(options, fileNames, fileIds, refresh){
        try{
                    ...
                    try{
                        ...
                        var fileM = $(fileLink.join('')).data('fileId', fileIdArr[i]).appendTo(container).menubutton({    
                            ...
                            if(isCanEdit == undefined || isCanEdit == null || isCanEdit == ""){
                                isCanEdit = isEditFile(options.editFileType, "", fileName);
    ...
    function genFileMenuHtml(menuId, options){
        try{
            ...
            if(options.isAdd && !options.triggerId){
               $("<div data-options=\"iconCls:'icon-add'\" name='fileAdd' item='true'>添加</div>").appendTo(menu);
            }
            if(options.isEdit){
               $("<div data-options=\"iconCls:'icon-edit'\" name='fileEdit' item='true'>编辑</div>").appendTo(menu);
            }else if(options.isView){
               $("<div data-options=\"iconCls:'icon-view'\" name='fileView' item='true'>查看</div>").appendTo(menu);
            }
            if(options.isDel){
               $("<div data-options=\"iconCls:'icon-delete'\" name='fileDel' item='true'>删除</div>").appendTo(menu);
            }
            if(options.isDownload){
               $("<div data-options=\"iconCls:'icon-export'\" name='fileDownload' item='true'>下载</div>").appendTo(menu);
            }
        }catch(e){
    ...
    */
		...
		toggleListStatus:function(jqTarget, isExpand){
        	try{
        		...
        		if(gridObj){
					...
					var gridTitle = options.fileGridTitle ? options.fileGridTitle : '';
					$('.layout-expand-'+targetRegion).find('.panel-title').text(gridTitle);
        		}
        	}catch(e){
    ...
    uploadObj.defaults = $.extend({}, uploadObj.defaults, {
        ...
        */
        ...
        multiple:true, 
        // 上传文件类型
        suffixs:"wps,pdf,ppt,pptx,doc,docx,xls,xlsx,zip,rar,txt,png,gif,jpg,bmp",
        // 不能上传的文件类型
        excludeSuffixs:"",
        // 每次最大上传文件个数， 默认：10个
        maxFileCount:10,
