...
@Date   :2018.8.31
@Author :qfucee
1.后台添加@TemplateField、@TemplateCombo、@FieldChecks、@FieldCheck标注，用于bean的导出模板，属性的校验，导出模版生成数据校验（下拉菜单）；
2.模板excel下载，根据指定的模板名称，读取bean是否有上面的标注，判断是否生成下拉，导入时是否根据配置进行属性的校验；
3.添加对空行的校验，当行前3个cell为空时，认为是空行，自动跳过；

@Version:1.4.1
@Date   :2018.11.27
@Author :qfucee
1.添加onBeforeOpen，打开前调用，返回true，打开窗口；返回false：取消打开；
    添加onBeforeClose 关闭前调用，返回true，关闭窗口，返回false：取消关闭;
*/
...
(function($){
    ...
    function init(target, options){
		try{
			if(target && options){
				...
				options.data.parentTabId = options.data.parentTabId?options.data.parentTabId:aud$getActiveTabId();
    ...
    function createTemplateForm(options){
    	try{
			...
			targetWin.$("<input type='hidden'></input>").attr({
				'name':'relation'
			}).val(options.relation).appendTo(templateForm[0]);
            
			targetWin.$("<input type='hidden'></input>").attr({
				'name':'beanName'
			}).val(options.beanName).appendTo(templateForm[0]);
			
			targetWin.$("<input type='hidden'></input>").attr({
				'name':'rowDataStartIndex'
			}).val(options.rowDataStartIndex).appendTo(templateForm[0]);
        ...
        try{
			...
			dialogContainer.dialog({
				...
				onBeforeClose:function(){
					return callFn(options, "onBeforeClose");
				},
				onClose:function(){
				...
				onBeforeOpen:function(){
					return callFn(options, "onBeforeOpen");
				},
				onOpen :function(){
					clearFileUploadInput(options);
    ...
    function callFn(options, fnName){
    	return options[fnName] ? options[fnName].call(this, options.target, options) : false;
    }
    ...
    parseExcel.methods = {
        ...
        downloadTemplate:function(jq){
        	...
        	options.targetWin.$(options.templateForm).form('submit',{
        		url:options.templateDownloadUrl,
