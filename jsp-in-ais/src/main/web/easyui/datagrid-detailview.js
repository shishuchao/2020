var detailview = $.extend({}, $.fn.datagrid.defaults.view, {
	...
	renderRow: function(target, fields, frozen, rowIndex, rowData){
		...
		for(var i=0; i<fields.length; i++){
			...
			if (col){
				...
				//lxf:隐藏expand列，自己控制
				if (field=='_expander') col.hidden = true;
				//lxf:end
	...
	onBeforeRender: function(target){
		...
		if (!hasExpander){
			...
			var td = $('<td rowspan="'+opts.frozenColumns.length+'"><div class="datagrid-header-expander" style="width:0px;"></div></td>');
			//lxf:end
			if ($('tr',t).length == 0){
...
$.extend($.fn.datagrid.methods, {
	...
	isExpandRow: function(jq, index){
		var expander = $(jq).datagrid('getExpander', index);
