...
<#if parameters.doubleSrc?exists>
 src="${parameters.contextPath}${parameters.doubleSrc?html}"<#rt/>
</#if>
<#if parameters.doubleOnclick?exists>
	<#assign onclick="${parameters.doubleOnclick}">
 	onclick="${onclick?replace("('","('${parameters.contextPath}")?string?replace("?url=","?url=${parameters.contextPath}")}"<#rt/>
</#if><#t/>
